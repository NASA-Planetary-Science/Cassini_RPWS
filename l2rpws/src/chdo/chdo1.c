...
struct chdo_ *chdo_ancillary_hdr (struct CDS_buffer *cds, struct chdo_ *chdo)
{
  switch (chdo->rec.header.type) {
   default:
     break;
   case 92:                            /* 0x5C */
     memcpy (&cds->packet.chdo_tag.ert, chdo->rec.secondary.ert, 6);
     memcpy (&cds->packet.chdo_tag.rct, chdo->rec.secondary.rct, 6);
     break;
   case 94:                            /* 0x5E */
     memcpy (&cds->packet.chdo_tag.sclk, chdo->rec.tertiary.sclk, 6);
     memcpy (&cds->packet.chdo_tag.scet, chdo->rec.tertiary.scet, 6);
     break;
   case 10:                            /* data */
     break;
  }
