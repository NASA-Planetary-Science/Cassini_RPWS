...
/* Look for special SOI mode with variable lengths. TFA 06-24-02 */
				...
				if(((m.buffer.packet.mpp.mini_packet[5] & 0x80) == 0x80) && (mp_len != 0x0405) && (mp_len != 0x0407) &&	/* 1Ksamp 
																																							 */
...
*/
							...
							for(ii = 0; ii < jam_it_index; ii++) {
								...
								dc = dc / (float) nsamp;
								for(i = 0; i < nsamp; i++) {	/* calibrate data */
							...
							if(num_to_write > 5461)
								num_to_write = 5461;	/* Limit! */
