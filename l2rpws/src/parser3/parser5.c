...
void print_help (FILE * otfile)
{
  ...
  fprintf (otfile,
           "\t-cdsseq_file name          Start Sequence and Set sequence-flag\n");
  fprintf (otfile,
  ...
  fprintf (otfile, "\t\t-cdsseq_file <name>\n");
                   ...
                   unsigned short *output_words)
{
  ...
  if (line[0] == '-') {
    ...
    if (strstr (line, "-cdsseq_file")) {
      ...
      fprintf (cdsseq_file, "#\n");
      fprintf (cdsseq_file, "secmd CDSSEQ BEGIN %s %04X:0 \n",
               cdsseq_name, cdsseq_time);
      fflush (cdsseq_file);
      cdsseq_time = 0;
    }
    /*
    ...
    if (strstr (line, "-cdsseq_activate")) {
      ...
      fprintf (cdsseq_file, "secmd CDSSEQ ACTIVATE %s %04X:0 \n",
               cdsseq_name, cdsseq_time);
      fflush (cdsseq_file);
  ...
  if (wrap (NULL) != NULL) {
    ...
    if (cdsseq_flag) {
      fprintf (cdsseq_file,
               ...
               cdsseq_name, time_offset - cdsseq_time, time_rti,
               cmd_sub_address, current->size + j);
