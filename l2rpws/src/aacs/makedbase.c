...
/*                                                                       */
/* 2012-10-17 CWP                                                        */
/*    Copied to SVN, added includes to get rid of compiler warnings      */
/*    Added help text                                                    */
/*    Original file location: /home/tfa/cassini/aacs/makedbase.c         */
...
/* Define the Fortran subroutine in quaternion.for */
void q2m_ (double quaternion[4], double matrix[3][3]);
...
main (int argc, char *argv[])
{
  ...
  int           byear, bdoy, bmon, bmday, bhr, bmn, bsclk;
  double	bsec;
  int           eyear, edoy, emon, emday, ehr, emn, esclk;
  double	esec;
  ...
  char          *ibuf;
	...
	if(argc > 1){
		if((strcmp(argv[1], "-h") == 0)||(strcmp(argv[1], "--help") == 0)){
	  		fprintf(stderr, "\n"
"rpws_quat_mkdb - Prints record times for Quaternion SFDU files\n"
"\n"
"DESCRIPTION\n"
"     This program reads the SFDU files from standard input and writes record\n"
"     times to standard output.  There are no command arguments, other than \n"
"     this help function.\n"
"\n"
"EXAMPLE\n"
"     cat t2012???0000.sdf | rpws_quat_mkdb > new_quaterions.txt\n"
"\n"
"FILES\n"
"     The output of this program is intended to be stored in\n"
"     $RPWS_DATA/database/Quaternions.db\n"
"\n");
			return 0;
		}
		else{
			fprintf(stderr, "Unknown command line arguments, use -h for more info\n");
			return 13;
		}
	}
 ...
 while (true)
 {
    ...
    {
      printf ("%4d-%3.3dT%2.2d:%2.2d:%06.3f %4d-%3.3dT%2.2d:%2.2d:%06.3f  %8X %8X  \n",
                 byear, bdoy, bhr, bmn, bsec,
                 eyear, edoy, ehr, emn, esec,
		 bsclk, esclk);
      return 0;
    }
    good=1;
    ...
    if (good)
    {
      sclk = (((unsigned int)buf[66])<<24) |
             (((unsigned int)buf[67])<<16) |
             (((unsigned int)buf[68])<<8)  |
              ((unsigned int)buf[69]);
