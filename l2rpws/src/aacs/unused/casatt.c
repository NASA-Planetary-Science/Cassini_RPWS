...
char    *fmakeword (FILE *f, char stop, int *len);
  ...
  int           J2000_FRAME=1;
  int           ECLIPJ2000_FRAME=17;

int process_data (double start_sec, double stop_sec, double deltat, struct file_structure *dbase, int n_files, char *att_type);
...
void ckgp_ (int *inst, double *sclk, double *tol, char *frame, double rmat[][], double *tickr, int *found, int framelen);

void att_print (double target_sec, double matrix[3][3],
                int no_data, char *att_type, char *data_source);
...
/***************************************************************/
   ...
   progname = argv [0];
   if (argc != 5)
   {    printf ("Syntax: %s start stop delta-t type\n", progname);
        printf ("        start and stop are standard time formats\n");
	printf ("        delta-t is time between output items in seconds\n");
	printf ("        type can be:\n"); 
	printf ("        quat   for Quaternions\n"); 
	printf ("        eule   for Euler Angles\n"); 
	printf ("        matr   for J2000 Transformation Matrix\n"); 
	printf ("        ugse   for for GSE Spacecraft Axes (Earth)\n"); 
	printf ("        uvse   for VSE Spacecraft Axes (Venus)\n"); 
	printf ("        ujse   for JSE Spacecraft Axes (Jupiter)\n"); 
	printf ("        usse   for SSE Spacecraft Axes (Saturn)\n"); 
	printf ("        ussq   for SSQ Spacecraft Axes (Saturn Equatorial)\n"); 
	printf ("        mecl   for Solar Ecliptic Spacecraft Axes (Sun)\n"); 
        exit(-1);
   }
...
*/ 
 ...
 furnsh_ ("/home/tfa/cassini/aacs/web/kernels.txt", 
          strlen ("/home/tfa/cassini/aacs/web/kernels.txt") );
...
/*
 ...
 fflush (stderr);
*/
 if ( !strcmp (argv[4], "uvse") )
   planet = Venus;
 else if ( !strcmp (argv[4], "ugse") )
   planet = Earth;
 else if ( !strcmp (argv[4], "ujse") )
   planet = Jupiter;
 else if ( !strcmp (argv[4], "usse") )
   planet = Saturn;
 else if ( !strcmp (argv[4], "ussq") )
    planet = SaturnBC;
 else					/* selected planet unrecognized */
   planet = Earth;

 if (process_data (start_sec, stop_sec, deltat, files, num_files, argv[4]) )
 {
   printf ("No data for the time range selected.\n");
 }
/*
 fprintf (stderr, " num_spice = %d \n", num_spice);
*/
 if (num_spice > 0)
 {
   printf (" \n");
...
int process_data (double start_sec, double stop_sec, double deltat, struct file_structure *dbase, int n_files, char *att_type)
{
  ...
  double	pkt_sec=0;
  static	int		st_inx, no_data, index;
  FILE		*finput;
...
/*
 fprintf (stderr, " Type of Attitude Info = %s \n", att_type);
*/
 if ( (!strcmp(att_type, "qecl") ) || (!strcmp(att_type, "mecl") )  )
    irfrot_ (&J2000_FRAME, &ECLIPJ2000_FRAME, J2000_to_ECLIP);
...
/* Calculate Euler angles */
	  ...
	  omega = atan2 (matrix[0][2], matrix[1][2]);
          ...
          phi = interp_angle (last_sec, pkt_sec, target_sec,
                              phi1,     phi2);
...
*/
          ckgp_ ( &inst, &tick0, &tol, frame, rmat,
	          &tickr, &found, strlen(frame));
...
/*		SPICE matrix from CKGP is transpose of AACS matrix. */
	  ...
	  att_print ( target_sec, matrix,
                      no_data, att_type, data_source);
	  no_data = False;
          target_sec = target_sec + deltat;
...
/* recalculate TICK0, in whatever the heck units SPICE uses */
          ...
          emitt ( target_sec, &year, &mon, &mday, &doy, &hr, &mn, &sec );
          sprintf (scetstr,"%4d-%3.3d // %2.2d:%2.2d:%6.3f",
                   year, doy, hr, mn, sec);
          utc2et_ (scetstr, &et, strlen(scetstr));
          sce2s_ ( &Cassini, &et, scstr, strlen(scstr));
          scencd_ (&Cassini, scstr, &tick0, strlen(scstr));
...
void make_dbase (double start_sec, double stop_sec, struct file_structure *dbase, int *num_files)
{
   ...
   char         *file_name = "/opt/project/cassini/data/database/Quaternions.db";
   FILE         *finput;
   char         sline [MAXCHARS];
...
/* Calculate Euler angles */
/*
	fprintf (stderr, "no_data=%d         att_type=%s\n", no_data, att_type);
*/
	  phi = atan2 (matrix[2][0], -matrix[2][1]);
	  delta = acos (matrix[2][2]);
	  omega = atan2 (matrix[0][2], matrix[1][2]);
...
/*			Rotate S/C X-axis		*/
	    ...
	    vec_in[1] = matrix[0][1];
	    vec_in[2] = matrix[0][2];
	    mxv_ (J2000_to_ECLIP, vec_in, vec_out);
...
/*			Convert to ECLIPJ2000 quaternions	*/
	    m2q_ (e_matrix, e_quaternion);
	    xpose_ (e_matrix, e_matrix);                                    /* get transpose */
	  }
	  ...
	  if (no_data)			/* first time only */
	  {
	    if ( !strcmp (att_type, "quat") )
            { printf ("         SCET           Quaternion1    Quaternion2");
              printf ("   Quaternion3   Quaternion4\n");
              printf ("---------------------   -----------    -----------");
              printf ("   -----------   -----------\n");
            }
            else if ( !strcmp (att_type, "qecl") )
	    { printf ("         SCET            EclipQuat1     EclipQuat2");
	      printf ("    EclipQuat3     EclipQuat4\n");
	      printf ("---------------------   -----------    -----------");
	      printf ("   -----------   -----------\n");
	    }
	    else if ( !strcmp (att_type, "eule") )
            { printf ("         SCET               Phi, deg.      Theta, deg.");
              printf ("     Omega, deg.     \n");
              printf ("---------------------     -----------      -----------");
              printf ("     -----------     \n");
            }
	    else if ( !strcmp (att_type, "matr") )
            { printf ("         SCET             T11,T21,T31      T12,T22,T32");
              printf ("     T13,T23,T33                \n");
              printf ("---------------------     -----------      -----------");
              printf ("     -----------     \n");
            }
            else if ( !strcmp (att_type, "mecl") )
            { printf ("         SCET             S/C X (ECL)      S/C Y (ECL)");
	      printf ("     S/C Z (ECL)\n");
	      printf ("---------------------     -----------      -----------");
	      printf ("     -----------     \n");
	    }
	    ...
	    {
              printf ("         SCET             S/C X (GSE)      S/C Y (GSE)");
              printf ("     S/C Z (GSE)\n");
              printf ("---------------------     -----------      -----------");
              printf ("     -----------     \n");
	    }
	    ...
	    {
              printf ("         SCET             S/C X (VSE)      S/C Y (VSE)");
              printf ("     S/C Z (VSE)\n");
              printf ("---------------------     -----------      -----------");
              printf ("     -----------     \n");
	    }
	    ...
	    {
              printf ("         SCET             S/C X (JSE)      S/C Y (JSE)");
              printf ("     S/C Z (JSE)\n");
              printf ("---------------------     -----------      -----------");
              printf ("     -----------     \n");
	    }
	    ...
	    {
              printf ("         SCET             S/C X (SSE)      S/C Y (SSE)");
              printf ("     S/C Z (SSE)\n");
              printf ("---------------------     -----------      -----------");
              printf ("     -----------     \n");
	    }
	    ...
	    {
	      printf ("         SCET             S/C X (SSQ)      S/C Y (SSQ)");
	      printf ("     S/C Z (SSQ)\n");
	      printf ("---------------------     -----------      -----------");
	      printf ("     -----------     \n");
	    }
	  ...
	  if ( !strcmp (att_type, "quat") )
            printf ("   %9.6lf      %9.6lf     %9.6lf     %9.6lf %s\n",
                                           ...
                                           quaternion[2],
                                           quaternion[3],
                                           quaternion[0], data_source);
	  else if ( !strcmp (att_type, "qecl") )
            printf ("   %9.6lf      %9.6lf     %9.6lf     %9.6lf %s\n",
                                           e_quaternion[1],
                                           e_quaternion[2],
                                           e_quaternion[3],
                                           e_quaternion[0], data_source);
	  else if ( !strcmp (att_type, "eule") )
            printf ("     %9.3lf        %9.3lf       %9.3lf %s\n",
					   phi, delta, omega, data_source);
	  else if ( !strcmp (att_type, "matr") )
          { printf ("     %9.6lf        %9.6lf       %9.6lf %s\n",
		     matrix[0][0], matrix[0][1], matrix[0][2], data_source);
            printf ("                       ");
	    printf ("     %9.6lf        %9.6lf       %9.6lf \n",
          ...
          { printf ("     %9.6lf        %9.6lf       %9.6lf %s\n",
		     e_matrix[0][0], e_matrix[0][1], e_matrix[0][2], data_source);
            printf ("                       ");
	    printf ("     %9.6lf        %9.6lf       %9.6lf \n",
		     e_matrix[1][0], e_matrix[1][1], e_matrix[1][2]);
            printf ("                       ");
            printf ("     %9.6lf        %9.6lf       %9.6lf \n",
		     e_matrix[2][0], e_matrix[2][1], e_matrix[2][2]);
            printf ("                          -----------      -----------");
            printf ("     -----------     \n");
 	  }
	  else if ( !strcmp (att_type, "ugse") ||
	  	    !strcmp (att_type, "uvse") ||
		    !strcmp (att_type, "ujse") ||
