...
static char command[128] = { "ls *_WFR_*" };
...
static int data_type = WFR_packet_ID_value;
...
char *now (int local_flag)
{
   ...
   ****	frequency fromt he WFR dsstatus			****
   ****							****
   *********************************************************/
char *frq_hfr_wfr (unsigned char status)
{
  static char text[64];
  float freq;
                      ...
                      struct DUST_STRUCT *dust_struct)
{
  ...
  switch (flag) {
   case 0:
     sprintf (time_file[index], "WFR_%s", gif_name);
     break;
   case 1:
     sprintf (time_file[index], "WFR_%s_26hz", gif_name);
     break;
   case 2:
     sprintf (time_file[index], "WFR_%s_2500hz", gif_name);
     break;
   case 4:
     sprintf (time_file[index], "WFR_%s_%s", gif_name, hfr_struct->band);
     break;
   case 8:
     ...
     temp_name[14] = '-';
     temp_name[17] = 0;
     sprintf (time_file[index], "WFR_%s_DUST", temp_name);
     break;
  }
...
int arrow2 (FILE * html_file, int count, char *page[], char *text[])
{
   ...
   ****	    wfr_mode is a mask of modes the WFR has	****
...
int make_html (char *start[],
               ...
               struct DUST_STRUCT *dust_struct,
               int wfr_mode, int count, char *argv[])
{
  int i;
  ...
  static char *filename[8] =
    { "wfr_html_file.html                                      ",
    "wfr_2500hz_html_file.html                                ",
    "wfr_26hz_html_file.html                                ",
    "wfr_hfr_html_file.html                                  ",
    "wfr_dust_html_file.html                                 ",
    "",
    "",
    ""
  };
  ...
  if (!html_file[0]) {
    if (start[0]) {
      ...
      sprintf (filename[1], "A_%s_2500.html",
               build_gif_name (0, gif_name, NULL, NULL));
      sprintf (filename[2], "A_%s_26.html",
               build_gif_name (0, gif_name, NULL, NULL));
    ...
    for (i = 0; i < 5; i++) {
      ...
      fprintf (html_file[i], "<META NAME=\"Keywords\" CONTENT=\"WFR\">\n");
      fprintf (html_file[i],
               "<META NAME=\"Keywords\" CONTENT=\"Quicklook\">\n");
      fprintf (html_file[i], "<TITLE>");
      fprintf (html_file[i], "    Cassini/RPWS WFR Quick Look\n");
      fprintf (html_file[i], "  </TITLE>\n");
      fprintf (html_file[i], "  </HEAD>\n");
      ...
      if (start[0]) {
        ...
        fprintf (html_file[i], "WFR Quicklook Dataset (%s)\n", gif_name);
        fprintf (html_file[i], "</H1>\n");
        switch (nert_flag) {
         case -1:
        ...
        sprintf (page_prev[1], "../%s/A_%s_2500.html", dtemp,
                 build_gif_name (0, ctemp, NULL, NULL));
        sprintf (page_prev[2], "../%s/A_%s_26.html", dtemp,
                 build_gif_name (0, ctemp, NULL, NULL));
        ...
        sprintf (page_next[0], "../%s/A_%s.html", dtemp,
                 build_gif_name (0, ctemp, NULL, NULL));
        sprintf (page_next[1], "../%s/A_%s_2500.html", dtemp,
                 build_gif_name (0, ctemp, NULL, NULL));
        sprintf (page_next[2], "../%s/A_%s_26.html", dtemp,
                 build_gif_name (0, ctemp, NULL, NULL));
        ...
        fprintf (html_file[i], "<H1>\n");
        fprintf (html_file[i], "WFR Quicklook Dataset\n");
        fprintf (html_file[i], "</H1>\n");
      }
      fprintf (html_file[i], "<br>\n");
  ...
  if (start[0]) {
    ...
    if (wfr_mode & 4) {
      for (index = 1; index < HFR_STRUCT_INDEX; index++) {
        if ((hfr_struct[index].band_edge[0]) &&
          ...
          fprintf (html_file[0],
                   "    <A HREF=\"http://cassini.physics.uiowa.edu/~tfa/cas/das-wfr2500hf2_new.html\">\n");
    ...
    if (wfr_mode & 2) {
      ...
      fprintf (html_file[0], "    <BR>\n");
      fprintf (html_file[0],
               "    <A HREF=\"http://cassini.physics.uiowa.edu/~tfa/cas/das-wfr2500_V4.html\">\n");
      fprintf (html_file[0], "    Web Interface\n");
      fprintf (html_file[0], "    </A>\n");
      fprintf (html_file[0], "    <P>\n");
      html_index[0]++;
    ...
    if (wfr_mode & 1) {
      ...
      fprintf (html_file[0], "    <BR>\n");
      fprintf (html_file[0],
               "    <A HREF=\"http://cassini.physics.uiowa.edu/~tfa/cas/das-wfr26_V4.html\">\n");
      fprintf (html_file[0], "    Web Interface\n");
      fprintf (html_file[0], "    </A>\n");
      fprintf (html_file[0], "    <P>\n");
      html_index[0]++;
    ...
    if (wfr_mode & 8) {
      for (index = 1; index < DUST_STRUCT_INDEX; index++) {
        if (dust_struct[index].time[0]) {
          ...
          fprintf (html_file[0], "    <BR>\n");
          fprintf (html_file[0],
                   "    <A HREF=\"http://cassini.physics.uiowa.edu/~tfa/cas/wfrplot.html\">\n");
          fprintf (html_file[0], "    Web Interface\n");
          fprintf (html_file[0], "    </A>\n");
          fprintf (html_file[0], "    <P>\n");
          html_index[0]++;
      ...
      if (image_count[i]) {
        ...
        switch (i) {
         ...
         case 1:
           fprintf (html_file[i], "<P>\n");
           fprintf (html_file[i], "It appears that we may not have any \n");
           fprintf (html_file[i], "2500hz data during this period.\n");
           break;
         case 2:
           fprintf (html_file[i], "<P>\n");
           fprintf (html_file[i], "It appears that we may not have any \n");
           fprintf (html_file[i], "26hz data during this period.\n");
           break;
         case 3:
           fprintf (html_file[i], "<P>\n");
           fprintf (html_file[i], "It appears that we may not have any \n");
           fprintf (html_file[i], "HF-WFR data during this period.\n");
           break;
         case 4:
           fprintf (html_file[i], "<P>\n");
           fprintf (html_file[i], "It appears that we may not have any \n");
                       ...
                       struct DUST_STRUCT *dust_struct)
{
 ...
 /****************************************************************************/
  ...
  switch (flag) {
   case 1:
     fprintf (time_file, "/browse_26hz.perl ");
     break;
   case 2:
     fprintf (time_file, "/browse_2500hz.perl ");
     break;
   case 4:
     fprintf (time_file, "/browse_HFR.perl ");
     break;
   case 8:
     fprintf (time_file, "/wfrplot ");
     break;
  }
  switch (flag) {
   case 1:
  ...
  switch (flag) {
   case 1:
     fprintf (time_file, "\"Cassini/RPWS WFR 26hz\" ");
     break;
   case 2:
     fprintf (time_file, "\"Cassini/RPWS WFR 2500hz\" ");
     break;
   case 4:
     fprintf (time_file, "\"Cassini/RPWS HF-WFR %s\" ", hfr_struct->band);
...
int make_script (char *start[],
                 char *stop[],
                 struct HFR_STRUCT *hfr_struct,
                 struct DUST_STRUCT *dust_struct, int wfr_mode, int count)
{
  char start_utc[64];
  char stop_utc[64];
  char dust_utc[64];
  ...
  static char filename[64] = { "wfr_time_file.sh" };
  static char move_file_name[64] = { "wfr_time_move.sh" };
  char *coordinate;
    ...
    if (start[0]) {
      if (0) {
        ...
        fprintf (time_file, "mv A_%s.html ${GIF_DIRECTORY}/%s/A_%s.html\n",
                 ...
                 build_gif_name (0, gif_name, hfr_struct, dust_struct));
        fprintf (time_file,
                 "mv A_%s_2500.html ${GIF_DIRECTORY}/%s/A_%s_2500.html\n",
                 build_gif_name (0, gif_name, hfr_struct, dust_struct),
                 subdir_name, build_gif_name (0, gif_name, hfr_struct,
                                              dust_struct));
        fprintf (time_file,
                 "mv A_%s_26.html ${GIF_DIRECTORY}/%s/A_%s_26.html\n",
                 build_gif_name (0, gif_name, hfr_struct, dust_struct),
                 subdir_name, build_gif_name (0, gif_name, hfr_struct,
                                              dust_struct));
        fprintf (time_file,
...
int browse_read_length_check (struct MP_buffer *buf)
{
  ...
  band = get_status (buf->packet.mpp.mini_packet, WFR_frequency_band, 0);
...
this_is_not_dust (struct MP_buffer * buf)
{
  ...
  msf = get_status (buf->packet.mpp.mini_packet, WFR_MSF, 0) & 0x01;
  msf *= 2;
  status = 1024;
  for (i = 0; i < 1024; i++) {
...
int main (int argc, char *argv[])
{
  ...
  if (buf) {
    while (buf) {
      ...
      /*
       * strcpy(hfr_struct[0].band, frq_hfr_wfr(get_status(buf->packet.mpp.mini_packet,
       * WFR_HFR_translate, 0) ) );/*
       */
      ...
      antenna = get_status (buf->packet.mpp.mini_packet, WFR_antenna, 0);
      band = get_status (buf->packet.mpp.mini_packet, WFR_frequency_band, 0);
      current_mode = 0;
      if (0) {                          /*(antenna == WFR_ANTENNA_HF)     /* HF-WFR */
        current_mode = 4;
        /*
         * new_wfr_mode |= current_mode; /*
         */
        fix_hfr_struct (hfr_struct, 0);
      } else if (band == WFR_BAND_2500HZ) {     /* band=1 for 2500hz */
        current_mode = 2;
        /*
         * new_wfr_mode |= current_mode; /*
         */
      } else if (this_is_not_dust (buf)) {      /* else band must bew 0 (26hz) */
        current_mode = 1;
        /*
         * new_wfr_mode |= current_mode; /*
         */
      ...
      if (1) {
        if (verbose_file) {
          fprintf (verbose_file, "%5d %X.%04X  %s\n", __LINE__, sclk.seconds,
                   sclk.fine >> 4, new_utc);
          fprintf (verbose_file,
                   "      26/2500/HF/Dust select  %02X=current_mode  %02X=wfr_mode  ",
                   current_mode, wfr_mode);
          fprintf (verbose_file, "%s", current_mode & 1 ? "26hz " : "");
          fprintf (verbose_file, "%s", current_mode & 2 ? "2500hz " : "");
          fprintf (verbose_file, "%s", current_mode & 4 ? "HF-WFR " : "");
      ...
      if (delta_t < max_delta) {
        ...
        if (stop_utc[0][0]) {
          if (verbose_file) {
            fprintf (verbose_file,
                     "%5d 26/2500/HF/Dust select  %02X=wfr_mode  ", __LINE__,
                     wfr_mode);
            fprintf (verbose_file, "%s", wfr_mode & 1 ? "26hz " : "");
            fprintf (verbose_file, "%s", wfr_mode & 2 ? "2500hz " : "");
            fprintf (verbose_file, "%s", wfr_mode & 4 ? "HF-WFR " : "");
            fprintf (verbose_file, "%s", wfr_mode & 8 ? "Dust " : "");
            fprintf (verbose_file, "  *************\n");
            fprintf (verbose_file,
                     "                                 to %s (%.3f) %d\n",
          ...
          make_html (start_utc,
                     stop_utc,
                     hfr_struct, dust_struct, wfr_mode, record_count, argv);
          make_script (start_utc,
                       stop_utc,
                       hfr_struct, dust_struct, wfr_mode, record_count);
          for (i = 1; i < 16; i++) {
            start_utc[i][0] = 0;
            stop_utc[i][0] = 0;
          }
        ...
        if (delta_t > 990000.)
          delta_t = 0.;
        if (verbose_file) {
          fprintf (verbose_file, "%5d WFR %10.3f %08X.%X  %s", __LINE__,
    ...
    if (stop_utc[0][0]) {
      ...
      make_html (start_utc, stop_utc, hfr_struct, dust_struct, wfr_mode,
                 record_count, argv);
      make_script (start_utc, stop_utc, hfr_struct, dust_struct, wfr_mode,
                   record_count);
      for (i = 1; i < 16; i++) {
        start_utc[i][0] = 0;
        stop_utc[i][0] = 0;
