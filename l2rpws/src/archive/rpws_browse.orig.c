...
static char Browse_File_Name[] = { "make_hr_browse.bat" };
static char path[] = { "/opt/project/cassini/archive" };
...
struct RPWS_LABEL *RPWS_BROWSE_data_record[16] =
...
static int rpws_browse_record (const struct RPWS_LABEL *detail_record,
                               const char *directory[],
                               const char *body_in, const int end_flag)
{
  ...
  if (first_time) {
    ...
    fprintf (time_file, "#\n");
    fprintf (time_file, "VOLUME_PATH=\"%s\"\n", path);
    fprintf (time_file, "VOLUME_SCRIPT=\"%s\"\n", script);
  ...
  switch (detail_record->instrument) {
   ...
   case RPWS_LABEL_WFR:
     fprintf (time_file, "cd %s\n",
              detail_record->
              instrument ? directory[RPWS_ARCHIVE_BROWSE_WFR] :
     ...
     if (detail_record->filepath3[0])
       fprintf (time_file, "cd %s\n", detail_record->filepath3);
     fprintf (time_file, "echo `pwd`/%s\n", filename);
     if (new_perl) {
       fprintf (time_file, "# Line: %d\n", __LINE__);
       fprintf (time_file, "if [ -f %s.PNG ]\n", thumbname);
       fprintf (time_file, "  then\n");
       fprintf (time_file, "    echo Exists %s.PNG\n", thumbname);
       fprintf (time_file, "  else\n");
       fprintf (time_file,
                "    "
                "${VOLUME_PATH}/${VOLUME_SCRIPT}/%s_browse_%s.perl %s \"%s\" \"%s\" \"%s\" \"%s\"",
                inst_label[detail_record->instrument],
                plot_band (detail_record),
                body, start_time, stop_time, title, orbit);
       if (detail_record->mode_mask & MODE_MASK_HFR) {
         int flower = 0;
         int fcenter = 0;
         int fupper = 0;
       ...
       fprintf (time_file,
                "    "
                "${VOLUME_PATH}/${VOLUME_SCRIPT}/%s_%s_browse%s.perl \"%s\" \"%s\" \"%s\" \"%s\"",
                inst_label[detail_record->instrument],
                inst_band[detail_record->fband],
                body, start_time, stop_time, title, orbit);
       fprintf (time_file, "\n");
     ...
     if (new_perl) {
       fprintf (time_file, "if [ -f HTML/%s_TN.PNG ]\n", thumbname);
       fprintf (time_file, "  then\n");
       fprintf (time_file, "    echo Exists HTML/%s_TN.PNG\n", thumbname);
       fprintf (time_file, "  else\n");
     }
     fprintf (time_file, "${VOLUME_PATH}/${VOLUME_SCRIPT}/thumbnail.sh %s\n",
              thumbname);
     if (new_perl) {
       fprintf (time_file, "  fi\n");
     }
     if (0)
       fprintf (time_file,
                "${VOLUME_PATH}/${VOLUME_SCRIPT}/move.sh %s %s %s\n",
                filename,
                detail_record->
                instrument ? directory[RPWS_ARCHIVE_BROWSE_WFR] :
                directory[RPWS_ARCHIVE_BROWSE_WBR], current_directory);
     if (detail_record->filepath1[0])
       fprintf (time_file, "cd ..\n");
     if (detail_record->filepath2[0])
       fprintf (time_file, "cd ..\n");
...
int rpws_browse (const char *directory[],       /* directory array */
                 char *body)
{                                       /* EARTH JUPITER SATURN, etc. */
  ...
  for (index = 1; index < 5; index++) {
    ...
    while (detail_record) {
      rpws_browse_record (detail_record, directory, body, 0);

/*           fprintf(stdout, "P %s %d %s %s %s\n", __FILE__, __LINE__, detail_record->filename, detail_record->scet_start, detail_record->scet_stop);  /**/
      detail_record = detail_record->link;
      count++;
    }
  }
  rpws_browse_record (NULL, directory, body, 1);
  return count;
}
...
char *rpws_browse_splat (char *buf)
{
  ...
  * 
  */
static int rpws_browse_html_record (struct RPWS_LABEL *detail_record,
                                    const char *directory,
                                    const char *sub_directory, const int flag)
{
  static FILE *html_file;
  static char html_filename[128];
  ...
  switch (flag) {
   ...
   case 1:                             /* WFR FREQ */
     if (sub_directory)
       sprintf (html_filename, "%s/%s/BROWSE_%sFR_FREQ.HTM", directory,
                sub_directory, Inst_Label[flag % 10]);
     else
       sprintf (html_filename, "%s/BROWSE_%sFR_FREQ.HTM", directory,
                Inst_Label[flag % 10]);
     break;
   case 10:                            /* WBR TIME */
   case 11:                            /* WFR TIME */
     if (sub_directory)
       sprintf (html_filename, "%s/%s/BROWSE_%sFR_TIME.HTM", directory,
                sub_directory, Inst_Label[flag % 10]);
     else
       sprintf (html_filename, "%s/BROWSE_%sFR_TIME.HTM", directory,
                Inst_Label[flag % 10]);
     break;
   case 20:                            /* WBR 10KHz    */
     if (sub_directory)
       sprintf (html_filename, "%s/%s/BROWSE_WBRFR_10KHZ.HTM", directory,
                sub_directory);
     else
   ...
   case 21:                            /* WFR 26Kz     */
     if (sub_directory)
       sprintf (html_filename, "%s/%s/BROWSE_WFRFR_26HZ.HTM", directory,
                sub_directory);
     else
   ...
   case 30:                            /* WBR 80Khz    */
     if (sub_directory)
       sprintf (html_filename, "%s/%s/BROWSE_WBRFR_80KHZ.HTM", directory,
                sub_directory);
     else
   ...
   case 31:                            /* WFR 2.5Khz   */
     if (sub_directory)
       sprintf (html_filename, "%s/%s/BROWSE_WFRFR_2500HZ.HTM", directory,
                sub_directory);
     else
  ...
  switch (flag) {
   ...
   case 20:
   case 30:
     fprintf (html_file, "WBR Browse Dataset,  ");
     break;
   case 1:
   case 11:
   case 21:
   case 31:
     fprintf (html_file, "WFR Browse Dataset,  ");
     break;
  }
  switch (flag) {
   case 0:
   case 1:
     fprintf (html_file, "Frequency Ordered\n");
     break;
   case 10:
   case 11:
     fprintf (html_file, "Time Ordered\n");
     break;
   case 20:
     fprintf (html_file, "WBR Low Band Subset\n");
     break;
   case 21:
     fprintf (html_file, "WFR Low Band Subset\n");
     break;
   case 30:
     fprintf (html_file, "WBR High Band Subset\n");
     break;
   case 31:
     fprintf (html_file, "WFR High Band Subset\n");
     break;
  }
  ...
  switch (flag) {
   ...
   case 31:
     fprintf (html_file, "  </H1>\n");
     fprintf (html_file,
              "Click on an image to view enlarged (full screen)\n");
...
/*             fflush(stdout);							/**/
  ...
  switch (flag) {
   case -2:
     ...
     fprintf (html_file, "<BR>\n");
     if (sub_directory)
       fprintf (html_file, "<A HREF=\"../../%s/%s\">", directory,
                sub_directory);
     else
     ...
     fprintf (html_file, "<BR>\n");
     fprintf (html_file,
              "Some spectrograms may be missing (i.e. they show up\n");
     fprintf (html_file,
              "with a <i>Data File Questionable</i> icon).  This is typically\n");
     fprintf (html_file,
              "due to corrupted data.  The <i>potentially</i> corrupt data\n");
     fprintf (html_file,
              "is retained in the archive in spite of the inability of the\n");
     fprintf (html_file,
              "display tools to generate a spectrogram for the period.\n");

     fprintf (html_file, "<BR><BR>\n");
     fprintf (html_file,
              "Occasional waveforms from the DUST analysis routine are \n");
     fprintf (html_file,
              "delivered to ground as WBR data.  This data is saved\n");
     fprintf (html_file,
              "in WBRFULL and marked to distinguish it from other WBR\n");
     fprintf (html_file,
              "data.  We do <b>not</b> attempt to produce spectrograms\n");
  ...
  switch (flag) {
   case -2:
     ...
     if (comment_flag & 0x100000) {
       fprintf (html_file,
                "       src=\"../../BROWSE/ANCILLARY/VALID_HTML401.PNG\"\n");
       fprintf (html_file,
                "       alt=\"Valid HTML 4.01!\" height=\"31\" width=\"88\">\n");
     } else {
       fprintf (html_file,
                "       src=\"../../BROWSE/ANCILLARY/VALID_HTML32.PNG\"\n");
       fprintf (html_file,
...
static int rpws_browse_html_record_1 (const char *directory,
                                      const char *sub_directory,
                                      const int band, const int index)
{
  int count = 0;
  struct RPWS_LABEL *detail_record;
...
int rpws_browse_sort (struct RPWS_LABEL **sorted_list,
                      struct RPWS_LABEL *list_1, struct RPWS_LABEL *list_2)
{
  int status = 0;
  ...
  while (temp) {
    new_member =
      malloc (sizeof (struct RPWS_LABEL) - sizeof (new_member->text_area));
...
int rpws_browse_html (char *directory[], int iflag,     /* 0=WBR 1=WFR */
                      int debug)
{
  int count = 0;
  int flag = 0;
  ...
  switch (iflag) {
   case RPWS_LABEL_WBR:
     rpws_browse_html_record (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              directory[RPWS_ARCHIVE_BROWSE_WBR], iflag + 10);
     count += rpws_browse_html_record_1 (directory[RPWS_ARCHIVE_BROWSE],
                                         directory[RPWS_ARCHIVE_BROWSE_WBR],
                                         -1, 6);
     rpws_browse_html_record (NULL, directory[iflag], NULL, -2);
   ...
   case RPWS_LABEL_WFR:
     rpws_browse_html_record (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              directory[RPWS_ARCHIVE_BROWSE_WFR], iflag + 10);
     count += rpws_browse_html_record_1 (directory[RPWS_ARCHIVE_BROWSE],
                                         directory[RPWS_ARCHIVE_BROWSE_WFR],
                                         -1, 5);
     rpws_browse_html_record (NULL, directory[iflag], NULL, -2);
  ...
  switch (iflag) {
   case RPWS_LABEL_WBR:
     rpws_browse_html_record (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              directory[RPWS_ARCHIVE_BROWSE_WBR], iflag + 20);
     count += rpws_browse_html_record_1 (directory[RPWS_ARCHIVE_BROWSE],
                                         directory[RPWS_ARCHIVE_BROWSE_WBR],
                                         ARCH_BAND_10KHZ, 6);
     rpws_browse_html_record (NULL, directory[iflag], NULL, -2);
     ...
     rpws_browse_html_record (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              directory[RPWS_ARCHIVE_BROWSE_WBR], iflag + 30);
     count += rpws_browse_html_record_1 (directory[RPWS_ARCHIVE_BROWSE],
                                         directory[RPWS_ARCHIVE_BROWSE_WBR],
                                         ARCH_BAND_75KHZ, 6);
     rpws_browse_html_record (NULL, directory[iflag], NULL, -2);
   ...
   case RPWS_LABEL_WFR:
     rpws_browse_html_record (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              directory[RPWS_ARCHIVE_BROWSE_WFR], iflag + 20);
     count += rpws_browse_html_record_1 (directory[RPWS_ARCHIVE_BROWSE],
                                         directory[RPWS_ARCHIVE_BROWSE_WFR],
                                         ARCH_BAND_25HZ, 5);
     rpws_browse_html_record (NULL, directory[iflag], NULL, -2);
     ...
     rpws_browse_html_record (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              directory[RPWS_ARCHIVE_BROWSE_WFR], iflag + 30);
     count += rpws_browse_html_record_1 (directory[RPWS_ARCHIVE_BROWSE],
                                         directory[RPWS_ARCHIVE_BROWSE_WFR],
                                         ARCH_BAND_2500HZ, 5);
     rpws_browse_html_record (NULL, directory[iflag], NULL, -2);
     ...
     /*******************************************
      *		PDS Label for PNG files		*
      *******************************************/
  switch (-1) {                         /* switch(iflag) /* */
   case -1:
     break;
   case RPWS_LABEL_WBR:
     rpws_label_browse_image (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              directory[RPWS_ARCHIVE_BROWSE_WBR], "", iflag);
     rpws_browse_PDS_label_1 (3, "");
     rpws_browse_PDS_label_1 (4, "");
     rpws_label_browse_image (NULL, NULL, NULL, "", -2);

     sprintf (temp, "%s/%s", directory[RPWS_ARCHIVE_BROWSE_WBR],
              directory[RPWS_ARCHIVE_BROWSE_HTML]);
     rpws_label_browse_image (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              temp, "_TN", iflag);
     rpws_browse_PDS_label_1 (3, "_TN");
     rpws_browse_PDS_label_1 (4, "_TN");
     rpws_label_browse_image (NULL, NULL, NULL, "_TN", -2);
   ...
   case RPWS_LABEL_WFR:
     rpws_label_browse_image (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              directory[RPWS_ARCHIVE_BROWSE_WFR], "", iflag);
     rpws_browse_PDS_label_1 (1, "");
     rpws_browse_PDS_label_1 (2, "");
     rpws_label_browse_image (NULL, NULL, NULL, "", -2);

     sprintf (temp, "%s/%s", directory[RPWS_ARCHIVE_BROWSE_WFR],
              directory[RPWS_ARCHIVE_BROWSE_HTML]);
     rpws_label_browse_image (NULL,
                              directory[RPWS_ARCHIVE_BROWSE],
                              temp, "_TN", iflag);
     rpws_browse_PDS_label_1 (1, "_TN");
     rpws_browse_PDS_label_1 (2, "_TN");
     rpws_label_browse_image (NULL, NULL, NULL, "_TN", -2);
