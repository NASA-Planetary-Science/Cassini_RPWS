...
static char *title = { " CASSINI MP data dump  (DSP5) 5.1" };
...
#define Millisecond 3
  switch ((mp[7] >> 5) & 0x03) {
   case Analysis:
     ...
     u_print0 (vert++, 0, string);
     sprintf (string, "ST_HFR ABC rep:%d integ:%sS DF:%s %s %s filt:%s ",
              mp[10],
              st_hfr (13, (mp[14] >> 6) & 0x03),
     ...
     u_print0 (vert++, 0, string);
     sprintf (string, "ST_HFR ABC cross:%s auto:%s ",
              st_hfr (2, (mp[23] >> 5) & 0x01),
              st_hfr (2, (mp[23] >> 2) & 0x01)
       );
     u_print0 (vert++, 0, string);
     sprintf (string, "ST_HFR HF1 rep:%d integ:%smS DF:%s %s %s filt:%s ",
              mp[11],
              st_hfr (16, (mp[15] >> 6) & 0x03),
     ...
     sprintf (string,
              "ST_HFR HF1 Freq:%dKhz-%dKhz  Steps:%d  deltaF:%dKhz cross:%s auto:%s ",
              startF, stopF, Nstep, deltaF, st_hfr (2, (mp[23] >> 4) & 0x01),
              st_hfr (2, (mp[23] >> 1) & 0x01)
       );
     u_print0 (vert++, 0, string);

     sprintf (string, "ST_HFR HF2 rep:%d integ:%smS DF:%s %s %s filt:%s ",
              mp[12],
              st_hfr (15, (mp[16] >> 6) & 0x03),
     ...
     stopF = startF + ((Nstep - 1) * deltaF);
     sprintf (string,
              "ST_HFR HF2 Freq:%dKhz-%dKhz  Steps:%d  deltaF:%dKhz cross:%s auto:%s",
              startF, stopF, Nstep, deltaF, st_hfr (2, (mp[23] >> 3) & 0x01),
...
main (int argc, char *argv[])
{
  ...
  if (fg_flag ("help") || fg_flag ("h")) {
    ...
    fprintf (stdout, "     -hex        supress hex dump\n");
    fprintf (stdout, "     -len        supress length line\n");
