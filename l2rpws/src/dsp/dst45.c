...
static char *title = { " CASSINI CDS data dump  (DSP4) 4.6" };
...
char *dsp4_pkt_sec (struct CDS_buffer *buffer)
{
  ...
  if (dsp4_pkt_delta (buffer)) {
    if ((dsp4_delta) == 1)
      sprintf (temp, " %d Pkt/sec %4.2f", count, dsp4_avg ());
    else
  ...
  while (ilen > 0) {
    ...
    if (hdr_flag) {
      ...
      if (cds[3] & 0xFF00)
        splat = 'E';
      sprintf (temp,
               " Cycle %5d Seq %c%5d CDS %4.4X %4.4X %4.4X %4.4X %4.4X %4.4X  Time 0x%8.4X.%X:%X  %c  %s %s",
               icnt, splat,
      ...
      if (thdr_flag)
        sprintf (temp,
                 ...
                 UTIL_extract_subRTI (buffer),
                 splat,
                 UTIL_extract_packet_brief (buffer), dsp4_pkt_sec (buffer));
