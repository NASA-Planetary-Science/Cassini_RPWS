...
static char *Version = { "CASSINI RPWS ROM housekeeping dump (HKROM4) V4.0" };
int bell_flag = 0;
int format_flag = 1;
int upkt_flag = 0;
static int epoch;
...
int eng_3 (struct CDS_buffer *buffer, int v_current)
{
  ...
  sprintf (temp, "  HFR %5.1fmA  +5:%2.3fV  +6:%2.3fV  -6:%2.3fV    ",
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 0),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 4),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 5),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 12));
  vert = mv_printw (vert, 0, temp);
  sprintf (temp, "  LRP %5.1fmA  +5:%2.3fV +12:%2.3fV    ",
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 3),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 11),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 10));
  vert = mv_printw (vert, 0, temp);
  sprintf (temp,
           "  MFR %5.1fmA  +5:%2.3fV  +6:%2.3fV +12:%2.3fV -12:%2.3fV  -6:%2.3fV    ",
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 1),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 8),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 7),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 6),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 13),
           eng3_conv (buffer->packet.rom_housekeeping.HFR_analog, 14));
  vert = mv_printw (vert, 0, temp);
...
int eng_disp (struct CDS_buffer *buffer, int v_current, int sort)
{
  ...
  i += 1;
  if (cnv)
    sprintf (temp, " Search Coil Temp %2.1fC  HALT %d%%    ",
             eng3a_conv (buffer->packet.rom_housekeeping.LRP_analog, i++),
             100 -
             (buffer->packet.rom_housekeeping.LRP_analog[15] * 100) / 255);
  else
 ...
 ********************************************************/
    if (UTIL_extract_CDS_length (&buffer) > 16)
      if (UTIL_extract_CDS_length (&buffer) < 200) {
        ...
        sprintf (temp,
                 "Seq%6.2d Len%4.2d epoch:%8X Time:%8X.%d Dsp: %d %d %d ",
        ...
        sprintf (temp,
                 "%4.4X %4.4X %4.4X %4.4X %4.4X %4.4X  %4d %3d %2.2d:%2.2d:%2.2d Out: %d %d %d ",
                 cds[0], cds[1], cds[2], cds[3], cds[4], cds[5],
                 pkt_tm->tm_year + 1900, pkt_tm->tm_yday, pkt_tm->tm_hour,
                 pkt_tm->tm_min, pkt_tm->tm_sec, ocnt[1], ocnt[2], ocnt[3]);
        mv_printw (4, 1, temp);
