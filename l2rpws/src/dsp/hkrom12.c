...
static char *Version = { "CASSINI RPWS housekeeping dump (HKROM12) V12.2" };
int bell_flag = 0;
int format_flag = 1;
int display_flag = 1;
int upkt_flag = 0;
...
int antenna_disp (struct CDS_buffer *buffer, int vert_in)
{
  ...
  if (ilen != 185)
    return 0;                           /* Hope 0 is okay here willy had no return value. ??? */
  vert = vert_in;
...
int main (int argc, char *argv[])
{
  ...
  if (fg_flag ("recent") == '-') {
    ...
    recentF = open (temp, O_WRONLY | O_CREAT);
    if (virtual_channel_flag) {
      recentF0 = open (temp0, O_WRONLY | O_CREAT);
      recentF1 = open (temp1, O_WRONLY | O_CREAT);
    }
