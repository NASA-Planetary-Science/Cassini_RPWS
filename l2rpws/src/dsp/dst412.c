...
static char *title = { " CASSINI CDS data dump  (DSP4) 4.12" };
...
raw_disp (struct CDS_buffer *buffer, int width, int flag)
{
  int index, i, j;
...
chdo_dump (unsigned char *buffer, int width, int index, char *text)
{
  int i, j;
  char temp[256];
...
chdo_dump_type_92 (struct CHDO_type_92 * buffer)
{
  int i, j;
  char temp[256];
  char *pub_text = { "      " };
...
chdo_dump_type_94 (struct CHDO_type_94 * buffer)
{
  int i, j;
  char temp[256];
...
main (int argc, char *argv[])
{
  ...
  if (fg_flag ("help") || fg_flag ("h")) {
    ...
    fprintf (stdout, "     -leap xx        Specify leap-year file (%s)\n",
             leapfile);
    fprintf (stdout, "     -sclk xx        Specify sclk/scet file (%s)\n",
             sclkfile);
  ...
  if (fg_flag ("leap")) {
    leapfile = malloc (256);
    strcpy (leapfile, fg_flagc ("leap"));
    fprintf (stdout, "leap second filename String: %s\n", leapfile);
    fflush (stdout);
  }
  if (fg_flag ("sclk")) {
    sclkfile = malloc (256);
    strcpy (sclkfile, fg_flagc ("sclk"));
    fprintf (stdout, "sclk/scet filename   String: %s\n", sclkfile);
    fflush (stdout);
  }
  ...
  while (ilen > 0) {
    ...
    if (hdr_flag) {
      ...
      if (lost_flag) {
        ...
        scs2e_ (&id, sclk_temp, &et, strlen (sclk_temp));
        et2utc_ (&et, format, &prec, utcout, strlen (format), 32);
        utcout[4] = 0;
        utcout[8] = 0;
        utcout[14] = 0;
        sprintf (temp, " C %5d %c%sSeq %5d " "CDS %4.4X %4.4X %4.4X %4.4X %4.4X %4.4X  " "%c" "Time 0x%8.4X.%X:%X(%X) " "%c  %s t%s%s%s00.r00", icnt, Spuke, spuke, UTIL_extract_CDS_sequence (buffer), cds[0], cds[1], cds[2], cds[3], cds[4], cds[5], tpuke, UTIL_extract_TIME (buffer), UTIL_extract_RTI (buffer), UTIL_extract_subRTI (buffer), UTIL_extract_Qflag (buffer), splat, UTIL_extract_packet_brief (buffer), &utcout[0], &utcout[5], &utcout[12]);       /* LOST RTI */
      } else
        sprintf (temp,
                 " Cycle %5d %c%sSeq %5d "
                 "CDS %4.4X %4.4X %4.4X %4.4X %4.4X %4.4X  "
                 "%c"
                 "Time 0x%8.4X.%X:%X(%X) "
                 "%c  %s %s",
                 icnt, Spuke, spuke,
