...
static char *Version = { "CASSINI RPWS housekeeping dump (HKROM10) V10.5" };
int bell_flag = 0;
int format_flag = 1;
int display_flag = 1;
int upkt_flag = 0;
...
int error_flag (struct CDS_buffer *buffer)
{
  ...
  if (temp & 0x0400)                    /* Reed Solomon error */
    index |= 4;
  if (temp & 0xC1E0)                    /* CDS error */
    index |= 2;
  if (temp & 0x3A00)                    /* FILL data */
    index |= 1;
  return index;
}
 ...
 ********************************************************/
    ...
    if (cds_length_valid) {
      ...
      if (epoch) {
        ...
        sprintf (temp,
                 "%4.4X %4.4X %4.4X %4.4X %4.4X %4.4X %4d-%3dT%2.2d:%2.2d:%2.2d.%3.3d Out: %3d %3d %3d ",
