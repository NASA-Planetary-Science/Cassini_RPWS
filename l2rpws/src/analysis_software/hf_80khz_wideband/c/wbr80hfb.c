...
static	double  a1HF1[4][A1_HF1_MAX_VALUES],a1HF2[4][A1_HF2_MAX_VALUES];
...
int process_agc (double start, double stop, double sec, struct file_structure *dbase, int n_files, double *MS_time, double *MS_freq, double *MS_agc);
...
/* do some rudimentary checks */
 ...
 read_A1HF_calibration_file("/opt/project/cassini/cal/version1.0/hfr/A1HF1.DAT",1);
 read_A1HF_calibration_file("/opt/project/cassini/cal/version1.0/hfr/A1HF2.DAT",0);
...
/*                                                              */
		 ...
		 rffti_ ( &nsamp, wsave );	/* initialize work array */
	       ...
	       { if ((m.buffer.packet.mpp.mini_packet[8] & 1) == 1)
		    ...
		    freq_offset = -62.5 + 4025.0 + 50.0 * (float)(m.buffer.packet.mpp.mini_packet[8]);
...
*/
	...
	spec[0] = offset + new_time + micro_offset;	/* spread them out */
	       ...
	       x_y_z[0] = spec[0];		/* seconds since start */
               ...
               return_status = process_agc (start_sec, stop_sec, requested_sec, dbase, num_files, &MS_time, &MS_freq, &MS_agc);
/*
	       if (return_status != 2)
	       {
...
*/
               ...
               else if ( ( (return_status == 2) && (MS_freq == HFWBR_freq) ) ||
...

int process_agc (double start_sec, double stop_sec, double requested_sec, struct file_structure *dbase, int n_files, double *MS_time, double *MS_freq, double *MS_agc)
{
...
not_first:
	 ...
	 if ( (!error_flag) && 
            (requested_sec <= (pkt_sec+DELTAT)) )  /* data ok ? */
	 {
	   ...
	   if ((m.buffer.packet.mpx.mini_packet[9] & 0xC0) == 0x40)
	   {
	     iant = 0;
	     ant_length = 5.0;				/* EX+ */
	   }
	   else
	   if ((m.buffer.packet.mpx.mini_packet[9] & 0xC0) == 0x80)
	   {
	     iant = 1;
	     ant_length = 5.0;				/* EX- */
	   }
	   else
	   if ((m.buffer.packet.mpx.mini_packet[9] & 0xC0) == 0xC0)
	   {
	     iant = 3;
	   ...
	   if ((m.buffer.packet.mpx.mini_packet[11] & 0x01) == 0x01)
	     band = 1;
...
FILE *handle;
  ...
  if(band>0){
    while((pLineIn=fgets(LineIn,512,handle))!=NULL){
      for(j=0;j<4;j++)  a1HF1[j][i]=next_double(&pLineIn);
      if(i++>=A1_HF1_MAX_VALUES)  break;
    }
  }
  else{
    while((pLineIn=fgets(LineIn,512,handle))!=NULL){
      for(j=0;j<4;j++)  a1HF2[j][i]=next_double(&pLineIn);
      if(i++>=A1_HF2_MAX_VALUES)  break;
    }
  }
