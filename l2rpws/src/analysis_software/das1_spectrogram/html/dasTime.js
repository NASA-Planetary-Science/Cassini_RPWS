...
//  else{ // if not PDS then count out 3 non-space delimiters 
//   sEndOfDate = sTime.split(this.NWS_DELIMITERS);
//    if(sEndOfDate.length >= 3)
//      sEndOfDate = sTime.indexOf(sEndOfDate[3]);
//    else
//      sEndOfDate = null;
//  }
  ...
  if( this.nYear < 1000 )  this.nYear += 1900;
  this.nMonth = 0;
  this.nMday = 0;
  this.nYday = 0;
  this.nHour = 0;
  ...
  for(i=0;i<sTokAr.length;i++){
    ...
    if(isNaN(nNumber) == true){ 
      if( sTokAr[i].length<3 || nWantDate==0 )  // No abbrevations < 3 chars
        return false;
      sTokAr[i] = sTokAr[i].toLowerCase();
      for(j=0; j<12; j++){  // search month array for a match
    ...
    if(nWantDate != 0){
      ...
      if(nNumber > 31){// Year 2000 bug (Y2K)
        if(nWantYear != 0){
          this.nYear = nNumber;
          if(this.nYear < 1000)
            this.nYear += 1900; 
...
// if(this.nMonth > 12) return false;

//  if(this.nMonth!=0 && this.nMday == 0) this.nMday = 1;
