...
/* Look for special RPXING mode. TFA 12-23-02 */
		...
		(mp_len != 0x0405) && (mp_len != 0x0407) &&      /* 1Ksamp */
		       ...
		       if ( ( mp_len & 0x3c00 ) > 0x1000 ) mp_len = 0x1007;
		   } 
               	   else
	           {   mp_len = (mp_len & 0x3c00) | 0x05;   /* MSF bit clear */
		       if ( ( mp_len & 0x3c00 ) > 0x1000 ) mp_len = 0x1005;
		   } 
	        }
               ...
               (mp_len == 0x1405) || (mp_len == 0x1407) )       /* 5Ksamp */
           {
             if ( ((pkt_sec >= start_sec) && (pkt_sec < stop_sec)) &&
                  ( (iant == 0) || (iant == 1) || (iant == 2)|| (iant == 4) ) &&     /* Ex or Bx (bogus cals) or LP (bogus cals) or Ez */
...
void make_dbase (double start_sec, double stop_sec, struct file_structure *dbase, int *num_files)
{
   ...
   *num_files = 0;
   do
   {
      if (fgets (in_line, MAXCHARS, finput) != NULL)
      {
         index = *num_files;
         sscanf (in_line, "%s %s %x %x %s %x",
                                     start, stop, 
                                     &begin_sclk, &end_sclk,
                                     (dbase+index)->line,
                                     &databits);
