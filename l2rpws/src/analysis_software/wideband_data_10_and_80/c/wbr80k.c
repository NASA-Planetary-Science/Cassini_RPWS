/*
        ...
        Modifications:
			...
			Mask MFR noise lines when at 70 dB gain. TFA 09/14/00
...
{
 int    epoch;
 int    inst_type, mp_len, modulus, if_index;
 int    wbrdata, nsamp, Nsamp, nlast=2048;
...
*/
 ...
 double pwr, ant_length, f_interfere;
 float  dc;
 float  xreal[2048];
...
/*
   fprintf (stderr, "Time: %4d %3.3d %2.2d:%2.2d:%6.3f \n", year,doy,hr,mn,sec);
*/
         ...
         if (pkt_sec < stop_sec)
         {
		  ...
		  deltaf = fs / 2048.;
	       ...
	       {
	         spec[2] = spec[1];
                 spec[1025] = (xreal[1023]*xreal[1023]) / bandw;
	       }
	       else
	           spec[1025] = (xreal[2047]*xreal[2047]) / bandw;
	       for (i=1; i<(nsamp/2); i++)
	       { if (Nsamp == 2048)
		 ...
		 {
		   spec[2*i+1] = (xreal[2*i-1]*xreal[2*i-1] + 
		                  xreal[2*i]*xreal[2*i]) / bandw;
		   spec[2*i+2] = spec[2*i+1];
		 }
	       }
/*
	If 70 dB gain and EX or EZ, mask MFR interference lines.
*/
	       if ( (igain == 70) && (iant != 3) )
	       {
	         modulus = mfr2mods[(int)(((RTI-1)&0x7F)/4)];
		 ...
		 { for (i=1; i<6; i++)
		   { spec[2*i*if_index] = spec[2*i*if_index-1];	 /* below */
		     spec[2*i*if_index+1] = spec[2*i*if_index-1];/* below */
		     spec[2*i*if_index+4] = spec[2*i*if_index+6];/* above */
		     spec[2*i*if_index+5] = spec[2*i*if_index+6];/* above */
		     spec[2*i*if_index+2] = 0.5 * (spec[2*i*if_index] + 
		     			           spec[2*i*if_index+4]); /* avg */
		     spec[2*i*if_index+3] = 0.5 * (spec[2*i*if_index] + 
		     			           spec[2*i*if_index+4]); /* avg */
		   }
		 }
	       }
