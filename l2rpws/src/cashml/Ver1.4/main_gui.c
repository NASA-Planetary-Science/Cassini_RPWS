...
MenuItem file_menu[]={ 
	...
	{"Log Data", &xmPushButtonWidgetClass,'P',NULL,NULL,LogData,    NULL,NULL},
	{"Seperator",&xmSeparatorWidgetClass,  0, NULL,NULL,0,        0,NULL},
...
XmString str;
  ...
  if(menu_type==XmMENU_PULLDOWN){
    ...
    cascade=XtVaCreateManagedWidget(menu_title,xmCascadeButtonWidgetClass,parent,
    	XmNsubMenuId,menu,XmNlabelString,str,XmNmnemonic,menu_mnemonic);
    XmStringFree(str); 
    }
...
static int nFileNumber;
  ...
  if(pLine==NULL){
    fprintf(stderr,"No Data to Log\n");
    return;
  }
  
  sprintf(sFileName,"cashml%03d.dat",nFileNumber); 
  ++nFileNumber;
  fprintf(stderr,"Filename %s, for data logging\n",sFileName);

  if((hHandle=fopen(sFileName,"wt"))==NULL){
    fprintf(stderr,"Unable to open %s, for data logging\n",sFileName);
    hHandle=stdout;
    return;  
  }
