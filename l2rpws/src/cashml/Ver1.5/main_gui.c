...
void FileCallback_Cancel(Widget w,XtPointer client_data,XtPointer call_data);
...
void AxisMinMax_GetValue(Widget w,XtPointer client_data,XtPointer call_data);
void AxisLogLin(Widget w,XtPointer client_data,XtPointer call_data);

void PlotResize(Widget w,XtPointer client_data,XEvent *event,Boolean *reg);
...
MenuItem log_menu[]={
  ...
  {"Log OnOff",&xmToggleButtonWidgetClass,'L',NULL,NULL,LogDataOnOff,NULL,NULL},
  {"Filename",&xmPushButtonWidgetClass,'F',NULL,NULL,LogDataFilename,NULL,NULL},
  NULL};


MenuItem file_menu[]={ 
	{"Open",     &xmPushButtonWidgetClass,'O',NULL,NULL,FileOpen, 0,NULL},
	{"Controls", &xmPushButtonWidgetClass,'C',NULL,NULL,PostPlayBackDialog,NULL,NULL},
	{"Seperator",&xmSeparatorWidgetClass,  0, NULL,NULL,0,        0,NULL},
	{"Print",    &xmPushButtonWidgetClass,'P',NULL,NULL,PrintWindow,NULL,NULL},
...
MenuItem lfdr_menu[]={ 
	{"Ex",  &xmToggleButtonWidgetClass,'x',NULL,NULL,AntSel_LFDR,(XtPointer)CasAntEx,NULL},
	{"Ez",  &xmToggleButtonWidgetClass,'E',NULL,NULL,AntSel_LFDR,(XtPointer)CasAntEz,NULL},
	{"Bx",  &xmToggleButtonWidgetClass,'B',NULL,NULL,AntSel_LFDR,(XtPointer)CasAntBx,NULL},
	{"By",  &xmToggleButtonWidgetClass,'y',NULL,NULL,AntSel_LFDR,(XtPointer)CasAntBy,NULL},
	{"Bz",  &xmToggleButtonWidgetClass,'z',NULL,NULL,AntSel_LFDR,(XtPointer)CasAntBz,NULL},
	{"LMR+",&xmToggleButtonWidgetClass,'+',NULL,NULL,AntSel_LFDR,(XtPointer)CasAntLMRp,NULL},
	{"LMR-",&xmToggleButtonWidgetClass,'-',NULL,NULL,AntSel_LFDR,(XtPointer)CasAntLMRm,NULL},
	{"LPs", &xmToggleButtonWidgetClass,'s',NULL,NULL,AntSel_LFDR,(XtPointer)CasAntLPs,NULL},
	NULL};

MenuItem mfr_menu[]={ 
	{"Ex",     &xmToggleButtonWidgetClass,'x',NULL,NULL,AntSel_MFR,(XtPointer)CasAntEx,NULL},
	{"Ez",     &xmToggleButtonWidgetClass,'E',NULL,NULL,AntSel_MFR,(XtPointer)CasAntEz,NULL},
	{"Bx",     &xmToggleButtonWidgetClass,'B',NULL,NULL,AntSel_MFR,(XtPointer)CasAntBx,NULL},
	{"Bz",     &xmToggleButtonWidgetClass,'z',NULL,NULL,AntSel_MFR,(XtPointer)CasAntBz,NULL},
	NULL};

MenuItem hfr_menu[]={ 
	{"Ex+",  &xmToggleButtonWidgetClass,'+',NULL,NULL,AntSel_HFR,(XtPointer)CasAntExp,NULL},
	{"Ex-",  &xmToggleButtonWidgetClass,'-',NULL,NULL,AntSel_HFR,(XtPointer)CasAntExm,NULL},
	{"Ex+/-",&xmToggleButtonWidgetClass,'x',NULL,NULL,AntSel_HFR,(XtPointer)CasAntEx,NULL},
	{"Ez",   &xmToggleButtonWidgetClass,'z',NULL,NULL,AntSel_HFR,(XtPointer)CasAntEz,NULL},
	NULL};
...
MenuItem axis_menu[]={ 
  {"Xmin",&xmPushButtonWidgetClass,  'i',NULL,NULL,AxisMinMax,(XtPointer)0,NULL},
  {"Xmax",&xmPushButtonWidgetClass,  'a',NULL,NULL,AxisMinMax,(XtPointer)1,NULL},
  {"Xlog", &xmToggleButtonWidgetClass,'o',NULL,NULL,AxisLogLin,(XtPointer)&FWall.bXlog,NULL},
  {"Seperator",&xmSeparatorWidgetClass,0, NULL,NULL,0,0,NULL},
  {"Ymin",&xmPushButtonWidgetClass,  'n',NULL,NULL,AxisMinMax,(XtPointer)2,NULL},
  {"Ymax",&xmPushButtonWidgetClass,  'x',NULL,NULL,AxisMinMax,(XtPointer)3,NULL},
  {"Ylog", &xmToggleButtonWidgetClass,'g',NULL,NULL,AxisLogLin,(XtPointer)&FWall.bYlog,NULL},
  NULL};
...
*/
  ...
  FWall.bNewTlmFile=False;  FWall.sFilename[0]='\0';
  FWall.pMainWindow=NULL;
  FWall.bXlog=True;      FWall.bYlog=True;
  FWall.fXleft=0.1;      FWall.fYtop=1.0E-3; 
  FWall.fXright=20.0E6;  FWall.fYbottom=1.0E-19;
...
*/
  ...
  wMainWindow=XtVaCreateWidget("mainwindow",xmMainWindowWidgetClass,wTopLevel,
...
XtManageChild(wMenuBar);
  ...
  wPane=XtVaCreateWidget("PannedWindow",xmPanedWindowWidgetClass,wMainWindow,NULL);
   n=0;
   XtSetArg(args[n],XmNheight,768);				n++;
   XtSetArg(args[n],XmNwidth,1024);				n++;
...
*/
  ...
  XtAddEventHandler(wTopLevel,StructureNotifyMask,False,PlotResize,(XtPointer)NULL);
  WorkId=XtAppAddWorkProc(AppContext,Work,(XtPointer)AppContext);
 
  XtVaSetValues(wMainWindow,XmNmenuBar,wMenuBar,XmNworkWindow,wPane,/*XmNcommandWindow,Command,*/NULL);
  WM_DELETE_WINDOW=XmInternAtom(XtDisplay(wTopLevel),"WM_DELETE_WINDOW",False);
...
XtPointer nTmp;
  ...
  for(i=0;i<wcnt;i++){
    if(!strcmp("Ex",XtName(pw[i])))        nAntenna=CasAntEx;
    else if(!strcmp("Ez",XtName(pw[i])))    nAntenna=CasAntEz;
    else if(!strcmp("Bx",XtName(pw[i])))    nAntenna=CasAntBx;
    else if(!strcmp("By",XtName(pw[i])))    nAntenna=CasAntBy;
    else if(!strcmp("Bz",XtName(pw[i])) )   nAntenna=CasAntBz;
    else if(!strcmp("LMR+",XtName(pw[i])))  nAntenna=CasAntLMRp;
    else if(!strcmp("LMR-",XtName(pw[i])))  nAntenna=CasAntLMRm;
    else if(!strcmp("LPs",XtName(pw[i])))   nAntenna=CasAntLPs;
  ...
  XtVaGetValues(wTmp,XmNchildren,&pw,XmNnumChildren,&wcnt,NULL);
  for(i=0;i<wcnt;i++){
    if(!strcmp("Ex+",XtName(pw[i])))        nAntenna=CasAntExp;
    else if(!strcmp("Ex-",XtName(pw[i])))   nAntenna=CasAntExm;
    else if(!strcmp("Ex+/-",XtName(pw[i]))) nAntenna=CasAntEx;
    else if(!strcmp("Ez",XtName(pw[i])) )   nAntenna=CasAntEz;
  ...
  for(i=0;i<wcnt;i++){
    state=False;
    if(!strcmp("Xlog",XtName(pw[i])))        state=FWall.bXlog;
    else if(!strcmp("Ylog",XtName(pw[i])))   state=FWall.bYlog; 
    if(state==True)
      XmToggleButtonSetState(pw[i],True,False);  
  }
/* 
...
int nLength=XtNumber(nXdata);
  ...
  if(bInit==False){
    bInit=True;   
    FWall.pMainWindow=New_CPGPlotWindow(xmp_device_name(wPgPlotWindow),1,1);

    if(FWall.bXlog==True){
      FWall.pMainWindow->w_xl=FWall.fXleft<=0.0?log10(0.1):log10(FWall.fXleft);
      FWall.pMainWindow->w_xr=FWall.fXright<=0.0?log10(16.1E6):log10(FWall.fXright);
    }
    else{
    ...
    if(FWall.bYlog==True){
      FWall.pMainWindow->w_yt=FWall.fYtop<=0.0?log10(1.0E-6):log10(FWall.fYtop);
      FWall.pMainWindow->w_yb=FWall.fYbottom<=0.0?log10(1.0E-19):log10(FWall.fYbottom);
    }
    else{
  ...
  if(FWall.bNewTlmFile==True){
    ...
    /* Assign File to Open */
    FWall.pTlmSrc->sNewInFile[FWall.pTlmSrc->nInFileIndex]=sEquals(FWall.sFilename); 
  }
...
/*      CasTelemetry_Dump(pPkt,stderr); */
    ...
    if(LoadWindow(FWall.pMainWindow)==True){  /* New data to display */
      cpgbbuf(); 
      FWall.pMainWindow->pLine=(CPGLine*)HeadListElement(FWall.pMainWindow->pLine);
...
*/
	...
	fprintf(FWall.hLogFile,"\n# EVTCLK %08X.%02X  RTI %04X  SCET %s\n",
          pPkt->sci.Time.evtclk.seconds,
          pPkt->sci.Time.evtclk.milliseconds,
          pPkt->sci.Time.rti,
	  CasTime_EvtToScetString(pPkt->sci.Time.evtclk.seconds,
	  pPkt->sci.Time.evtclk.milliseconds));
	...
	switch(pPkt->nType&CasSciMask){
          ...
          case CasSciLFDR :
            nLen=CasLFDR_GetPhysicalUnits(&(pPkt->lfdr),
                 FWall.nLFDRmode,fMag,fFrq,NULL);
            if(nLen>0)
	    ...
	    break;
		 ...
		 (FWall.nHFRmode&CasHFR_BandMask)|
		 (FWall.nHFRmode&CasAntEx),fMag,fFrq,NULL);
            if(nLen>0)
	    ...
	    for(i=0;i<nLen;i++){
	      ...
	      fprintf(FWall.hLogFile,"%#-4.4E      %#-4.4E\n",
		   fFrq[i],fMag[i]); 
            ...
            nLen=CasHFR_GetPhysicalUnits(&(pPkt->hfr),
		 (FWall.nHFRmode&CasHFR_BandMask)|
		 (FWall.nHFRmode&CasAntEz),fMag,fFrq,NULL);
            if(nLen>0)
	    ...
	    for(i=0;i<nLen;i++){
	      ...
	      fprintf(FWall.hLogFile,"%#-4.4E      %#-4.4E\n",
		   fFrq[i],fMag[i]); 
            ...
            nLen=CasHFR_GetPhysicalUnits(&(pPkt->hfr),
		 (FWall.nHFRmode&CasHFR_BandMask)|
		 (FWall.nHFRmode&(CasAntExp|CasAntExm)),fMag,fFrq,NULL);
            if(nLen>0)
...
/* LFDR - lfdr - LFDR - lfdr - LFDR - lfdr - LFDR - lfdr -  LFDR */
  ...
  for(nLoop=0;nLoop<8;nLoop++){
    ...
    switch(nLoop){
      case 0 :  nAnt=CasAntEx;    break;
      case 1 :  nAnt=CasAntEz;    break;
      case 2 :  nAnt=CasAntBx;    break;
      case 3 :  nAnt=CasAntBy;    break;
      case 4 :  nAnt=CasAntBz;    break;
      case 5 :  nAnt=CasAntLMRp;  break;
      case 6 :  nAnt=CasAntLMRm;  break;
      case 7 :  nAnt=CasAntLPs;   break;
      default : nAnt=CasAntEx;    break;
    }
    ...
    if((FWall.pLFDR[nLoop]!=NULL) && (FWall.nLFDRmode&nAnt)){
      ...
      while(pLine!=NULL){
        ...
        if((pLineId->nType&CasSciLFDR) && (pLineId->nMode&nAnt)){ 
          if(pLineId->nEvtSec!=FWall.pLFDR[nLoop]->lfdr.Time.evtclk.seconds){
            bNewData=True;
            ...
            nLen=CasLFDR_GetPhysicalUnits(&(FWall.pLFDR[nLoop]->lfdr),
                      CasLFDR_BandsAll|nAnt,pLine->y,pLine->x,NULL);
            pLineId=(PacketId*)pLine->pUserData; 
        ...
        if((pLineId->nType&CasSciMFR) && (pLineId->nMode&nAnt)){ 
          if(pLineId->nEvtSec!=FWall.pMFR[nLoop]->mfr.Time.evtclk.seconds){
            ...
            nLen=CasMFR_GetPhysicalUnits(&(FWall.pMFR[nLoop]->mfr),
                      CasMFR_BandsAll|nAnt,pLine->y,pLine->x,NULL);
            pLineId=(PacketId*)pLine->pUserData; 
...
/* HFR - hfr - HFR - hfr - HFR - hfr - HFR - hfr - HFR - hfr - HFR */
  ...
  /* Look in Window List to see if new line need to be created */
  for(nLoop=0;nLoop<4;nLoop++){
    if(nLoop==0)       nAnt=CasAntExp;
    else if(nLoop==1)  nAnt=CasAntExm;
    else if(nLoop==2)  nAnt=CasAntEx;
    else if(nLoop==3)  nAnt=CasAntEz;
        ...
        if((pLineId->nType&CasSciHFR) && (pLineId->nMode&nAnt)){ 
          if(pLineId->nEvtSec!=FWall.pHFR[nLoop]->hfr.Time.evtclk.seconds){
            ...
            nLen=CasHFR_GetPhysicalUnits(&(FWall.pHFR[nLoop]->hfr),
                      CasHFR_BandsAll|nAnt,pLine->y,pLine->x,NULL);
            pLineId=(PacketId*)pLine->pUserData; 
...
static char sHFR[256],sMFR[256],sLFDR[256];
  ...
  for(i=0;i<8;i++){
    if(FWall.pLFDR[i]!=NULL){
      switch(i){
        case 0 :  strcat(sLFDR,"Ex ");  break;
        case 1 :  strcat(sLFDR,"Ez ");  break;
        case 2 :  strcat(sLFDR,"Bx ");  break;
        case 3 :  strcat(sLFDR,"By ");  break;
        case 4 :  strcat(sLFDR,"Bz ");  break;
        case 5 :  strcat(sLFDR,"LMR+ ");  break;
        case 6 :  strcat(sLFDR,"LMR+ ");  break;
        case 7 :  strcat(sLFDR,"LPs ");  break;
      }
  ...
  for(i=0;i<4;i++){
    if(FWall.pMFR[i]!=NULL){
      switch(i){
        case 0 :  strcat(sMFR,"Ex ");  break;
        case 1 :  strcat(sMFR,"Ez ");  break;
        case 2 :  strcat(sMFR,"Bx ");  break;
        case 3 :  strcat(sMFR,"Bz ");  break;
      }
  ...
  for(i=0;i<4;i++){
    if(FWall.pHFR[i]!=NULL){
      switch(i){
        case 0 :  strcat(sHFR,"Ex+ ");  break;
        case 1 :  strcat(sHFR,"Ex- ");  break;
        case 2 :  strcat(sHFR,"Ex+/- ");  break;
        case 3 :  strcat(sHFR,"Ez ");  break;
      }
  ...
  if(pPktId==NULL)
    ...
    if(FWall.bXlog==True){
      for(nLoop=0;nLoop<pLine->length;nLoop++)
        pLine->x[nLoop]=pLine->x[nLoop]<=0.0?log10(1.0E-120):log10(pLine->x[nLoop]);
    }
    if(FWall.bYlog==True){
      for(nLoop=0;nLoop<pLine->length;nLoop++)
        pLine->y[nLoop]=pLine->y[nLoop]<=0.0?log10(1.0E-120):log10(pLine->y[nLoop]);
    }
  }
  else if(pPktId->nType&CasSciHFR){  /* Default Units are dBVrms/rtHz */
    if(pPktId->nMode&CasAntEx)  fTmp=1.0/9.26;  
    else                        fTmp=1.0/5.00;  
...
XmString str;
  ...
  if(menu_type==XmMENU_PULLDOWN){
    ...
    cascade=XtVaCreateManagedWidget(menu_title,xmCascadeButtonWidgetClass,
         parent,XmNsubMenuId,menu,XmNlabelString,str,XmNmnemonic,
	 menu_mnemonic);
    XmStringFree(str); 
    }
  ...
  for(i=0;items[i].label!=NULL;i++){
    ...
    if(items[i].subitems){
      ...
      if(items[i].widget_class==NULL){     /* Elicit RadioButton Respones */
        ...
        widget=XtVaCreateManagedWidget(items[i].label,*items[i].widget_class,
	     menu,NULL);
        if(i==0)  
...
void PopupErrorDialog(Widget w,XtPointer client_data,XtPointer call_data)
{
XmString sMsg=XmStringCreateLtoR((char*)client_data,XmFONTLIST_DEFAULT_TAG);
XmString sTit=XmStringCreateLtoR((char*)"Error",XmFONTLIST_DEFAULT_TAG);
Widget wDialog;
...
Widget wDialog,wShell,wMenu,wParent;
  ...
  if(user_data==NULL){
    ...
    n=0;
    XtSetArg(args[n],XmNdirectory,Str);  ++n;

    wMenu=w;
    wParent=w;
    wDialog=XmCreateFileSelectionDialog(wParent,"file_selection_dialog",args,n);/**/
...
XmStringFree(Str);
    ...
    XtAddCallback(wDialog,XmNokCallback,FileCallback_Ok,(XtPointer)wShell);
    XtAddCallback(wDialog,XmNcancelCallback,FileCallback_Cancel,(XtPointer)wShell);
    WM_DELETE_WINDOW=XmInternAtom(XtDisplay(wShell),"WM_DELETE_WINDOW",False);
    XmAddWMProtocolCallback(wShell,WM_DELETE_WINDOW,FileCallback_Cancel,(XtPointer)wShell);
    XtAddCallback(wShell,XmNpopdownCallback,ToggleSensitivity,(XtPointer)wMenu);
    XtAddCallback(wShell,XmNpopupCallback,ToggleSensitivity,(XtPointer)wMenu);
    XtManageChild(wDialog);
...
void DE(Widget w,XtPointer client_data,XtPointer call_data);
  ...
  if(!Dialog){
    Caller=w;
    while(wms && !XtIsWMShell(wms))  wms=XtParent(wms);
    Dialog=XtVaCreatePopupShell("dialog",topLevelShellWidgetClass,wms,XmNdeleteResponse,XmDO_NOTHING,NULL);
/*    Dialog=XtVaCreatePopupShell("dialog",xmDialogShellWidgetClass,wms,XmNdeleteResponse,XmDO_NOTHING,NULL);*/
    pane=XtVaCreateWidget("pane",xmPanedWindowWidgetClass,Dialog,XmNsashWidth,1,XmNsashHeight,1,XmNseparatorOn,False,NULL);
...
------------------*/
    ...
    rowcolumn=XtVaCreateWidget("rowcolumn",xmRowColumnWidgetClass,pane,XmNorientation,XmHORIZONTAL,
    ...
    for(i=0;i<num_btn;i++){
      ...
      if( i==1 || i==3 || i==5 ){		/* StepRewind,Stop,StepForward */
        pushbutton=XtVaCreateManagedWidget("pushbutton",xmPushButtonWidgetClass,rowcolumn,
    	  XmNlabelType,XmPIXMAP,XmNlabelPixmap,image,NULL);
        XtAddCallback(pushbutton,XmNactivateCallback,PlayBack,(XtPointer)i);
        }
      else{
        drawnbutton=XtVaCreateManagedWidget("drawnbutton",xmDrawnButtonWidgetClass,rowcolumn,
    	  XmNlabelType,XmPIXMAP,XmNlabelPixmap,image,
    	  XmNpushButtonEnabled,False,XmNshadowType,XmSHADOW_OUT,NULL);
    	if(i==4)  default_pb=drawnbutton;	/* Default Button Play i=4 */
        XtAddCallback(drawnbutton,XmNactivateCallback,PlayBack,(XtPointer)i);
    ...
    XmAddWMProtocolCallback(Dialog,WM_DELETE_WINDOW,PostPlayBackDialog,NULL);
...
static Widget wlast;
  ...
  switch(btn_num){
    case 0 :	
	 FWall.pTlmSrc->nDataDirection=RecorderDirectionFastRewind;   break;
    case 1 :
	 FWall.pTlmSrc->nDataDirection=RecorderDirectionStepRewind;   break;
    case 2 :
	 FWall.pTlmSrc->nDataDirection=RecorderDirectionPlayRewind;   break;
    case 3 :
	 FWall.pTlmSrc->nDataDirection=RecorderDirectionStop;         break;
    case 4 :
	 FWall.pTlmSrc->nDataDirection=RecorderDirectionPlayForward;  break;
    case 5 :
	 FWall.pTlmSrc->nDataDirection=RecorderDirectionStepForward;  break;
    case 6 :
	 FWall.pTlmSrc->nDataDirection=RecorderDirectionFastForward;  break;
    default :
	 FWall.pTlmSrc->nDataDirection=RecorderDirectionPlayForward;  break;
      break;
    }
...
XConfigureEvent *cfgevt=(XConfigureEvent*)event;
  ...
  if(FWall.pMainWindow==NULL)  return;
...
static int nFileNumber;
  ...
  while(pLine!=NULL){
	...
	for(i=0;i<16;i++){  /* Band 1 */
          dX =pLine->x[i];     dY =pLine->y[i];
          dX1=pLine->x[i+16];  dY1=pLine->y[i+16];
          if(FWall.bXlog==True){  dX = pow(10,dX); dX1 = pow(10,dX1);}
          if(FWall.bYlog==True){  dY = pow(10,dY); dY1 = pow(10,dY1);}
	  if(dX==dX1){
	...
	pX=&(pLine->x[32]);  pY=&(pLine->y[32]);
	for(i=0;i<32;i++){  /* Band 2 */
          dX =pX[i];     dY =pY[i]; 
	  dX1=pX[i+32];  dY1=pY[i+32];
          if(FWall.bXlog==True){  dX = pow(10,dX); dX1 = pow(10,dX1);}
          if(FWall.bYlog==True){  dY = pow(10,dY); dY1 = pow(10,dY1);}
	  if(dX==dX1){
            fprintf(hHandle,"%#-4.4E   %#-4.4E   %#-4.4E\n",dX,dY,dY1);
	...
	pX=&(pLine->x[32+64]);  pY=&(pLine->y[32+64]);
	for(i=0;i<32;i++){  /* Band 3 */
          dX=pX[i];  dX1=pX[i+32];  dX2=pX[i+64];  dX3=pX[i+96];  
          dY=pY[i];  dY1=pY[i+32];  dY2=pY[i+64];  dY3=pY[i+96];  
          if(FWall.bXlog==True){  
	    dX = pow(10,dX);  dX1 = pow(10,dX1);
	    dX2= pow(10,dX2); dX3 = pow(10,dX3);
	  }
          if(FWall.bYlog==True){
	    dY  = pow(10,dY);  dY1 = pow(10,dY1);
	    dY2 = pow(10,dY2); dY3 = pow(10,dY3);
	  }
	  if((dX==dX1) && (dX==dX2) && (dX==dX3)){
            fprintf(hHandle,"%#-4.4E   %#-4.4E   %#-4.4E   %#-4.4E   %#-4.4E\n",
		   dX,dY,dY1,dY2,dY3);
	  }
          else{
            fprintf(hHandle,"%#-4.4E   %#-4.4E\n",dX, dY);
	...
	OutputLineData(hHandle,pLine);
      ...
      fprintf(hHandle,"# HFR %s, Event Seconds %08X\n",
        ...
        for(i=0;i<pLine->length;i++){
          dX=pLine->x[i];
          dY=pLine->y[i];
          if(FWall.bXlog==True)  dX = pow(10,dX);	
          if(FWall.bYlog==True)  dY = pow(10,dY);	
          fprintf(hHandle,"%#-4.4E   %#-4.4E\n",dX,dY);
        }
        fprintf(hHandle,"\n");
    }/* if hfr data */
...
void LogDataOnOff(Widget w,XtPointer client_data,XtPointer call_data)
{
XmToggleButtonCallbackStruct* pCbs=(XmToggleButtonCallbackStruct*)call_data;
char sTmp[64];
...
XmString sMsg=XmStringCreateLocalized("Enter the filename for data logging:");
...
int n=0;
  ...
  XtSetArg(args[n],XmNtextString,Str1);  ++n;
  XtSetArg(args[n],XmNautoUnmanage,False);        ++n;
  wDialog=XmCreatePromptDialog(w,"Axis",args,n);
  XtAddCallback(wDialog,XmNokCallback,AxisMinMax_GetValue,(XtPointer)pValue);
  XtAddCallback(wDialog,XmNcancelCallback,XtDestroyWidget,NULL);
  XtSetSensitive(XmSelectionBoxGetChild(wDialog,XmDIALOG_HELP_BUTTON),False);
  XtManageChild(wDialog);
  XtPopup(XtParent(wDialog),XtGrabNone);
...
PacketId *pLineId=(PacketId*)pLine->pUserData;
  ...
  if(pLineId->nMode&CasAntEx)         pLine->color_index=1;		/* White */
  else if(pLineId->nMode&CasAntEz)    pLine->color_index=2;		/* Red */
  else if(pLineId->nMode&CasAntBx)    pLine->color_index=3;		/* */
  else if(pLineId->nMode&CasAntBy)    pLine->color_index=4;		/* */
  else if(pLineId->nMode&CasAntBz)    pLine->color_index=5;		/* */
  else if(pLineId->nMode&CasAntLMRp)  pLine->color_index=6;		/* */
  else if(pLineId->nMode&CasAntLMRm)  pLine->color_index=7;		/* */
  else if(pLineId->nMode&CasAntLPs)   pLine->color_index=8;		/* */
  else                                pLine->color_index=9;		/* */
...
PacketId *pLineId=(PacketId*)pLine->pUserData;
  ...
  if(pLineId->nMode&CasAntExp)       pLine->color_index=10;		/* White */
  else if(pLineId->nMode&CasAntExm)  pLine->color_index=11;		/* Red */
  else if(pLineId->nMode&CasAntEx)  pLine->color_index=1;		/* */
  else if(pLineId->nMode&CasAntEz)  pLine->color_index=2;		/* */
  else                              pLine->color_index=12;		/* */
