...
static char *step_[] = { "STEP_",
  ...
  "STEP_Segmented_missing",
  "STEP_Continue",
  "STEP_Error" "STEP_Error" "STEP_Error"
};
...
struct MP
{
  ...
  int mindex;                           /* index into mini packet */
  unsigned char flag_1;                 /*                        */
  unsigned char flag_2;                 /*                        */
  unsigned char flag_3;                 /*                        */
  unsigned char flag_4;                 /*                        */
  struct DATA_RECORD *data_buffer;      /* cds data structure     */
  struct MP_buffer *mp_buffer;          /* mp dta structure       */
};
static struct MP mp[MP_THREADS + 1] = { 0, 0, 0, 0, 0, 0, NULL, NULL,
...
void MP2_MP_dump (struct MP *mp, FILE * fout, int flag, int index)
{
  ...
  if (flag & 0x04) {
    ...
    fprintf (fout,
             "    cds_hdr   %02X%02X %02X%02X %02X%02X %02X%02X %02X%02X %02X%02X\n",
    ...
    fprintf (fout,
             "    rpws_data %4d: %02X %02X %02X %02X %02X %02X %02X %02X\n",
             index, mp->data_buffer->cds_record.packet.rpws.data[index + 0],
  ...
  if (flag & 0x08) {
    ...
    fprintf (fout,
             "    mp_data  %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X",
  ...
  if (flag & 0x20) {
    ...
    fprintf (fout, "%02X%02X%02X%02X.%02X ",
    ...
    fprintf (fout, "(%4X) ", mp->mp_buffer->packet.index.data_start);
    fprintf (fout, "%02X %02X%02X %02X%02X ",
             type,
             mp->mp_buffer->packet.mpp.mini_packet[0] & 0x0F,
             mp->mp_buffer->packet.mpp.mini_packet[1],
             mp->mp_buffer->packet.mpp.mini_packet[3],
             mp->mp_buffer->packet.mpp.mini_packet[2]
      );
  ...
  if (flag & 0x40) {
    fprintf (fout, "%02X%02X %02X%02X %02X%02X %02X %8X.%d",
             mp->data_buffer->cds_record.packet.cds.header[1],
             mp->data_buffer->cds_record.packet.cds.header[0],
...
int MP2_mp_count (struct MP *mp, int index, int sw)
{
  ...
  count_cds =
    ((mp->data_buffer->cds_record.packet.cds.header[5] & 0x3F) << 8) | (mp->
                                                                        data_buffer->
                                                                        cds_record.
                                                                        packet.
                                                                        cds.
  ...
  *			move data
  *******************************************************/
int MP2_mp_move (struct MP *mp, int index, int skip)
{
  int count = 0;
  int type;
  ...
  memcpy (&mp->mp_buffer->packet.mpp.mini_packet[mp->mindex],
...
void MP2_mp_clear (struct MP *mp)
{
 ...
 /*******************************************************
  *	Write and MP data to the output stream		*
...
void MP2_flush (FILE * mpout, struct MP *mp)
{
  ...
  length += 3;
			 ...
			 ************************************************/
  mp->mp_buffer->packet.index.data_start -= 3;
  ...
  if (length > 3) {
    ...
    mp->mp_buffer->record_type =
      MP2_mp_type_valid (mp->mp_buffer->packet.mpp.mini_packet[0], 0);
			 ...
			 ************************************************/
    flag = 0x130;
    if (MP2_mp_stim_check (mp))
      if (MP2_filter (mp->mp_buffer)) {
  ...
  while (busy) {
    switch (mp->flag_1) {
     case STEP_Unsegmented:
       MP2_flush (mpout, mp);           /* flush existing data  */
       if (!MP2_mp_validate (mp, 0))
         return;
       cindex = MP2_mp_move (mp, cindex, 0);    /* move this packet     */
       if (cindex < 0) {                /* need more data */
         mp->sequence = new_sequence & 0x3FFF;
         return;
       }
       mp->flag_1 = STEP_Continue;      /* indicate 'carry on'  */
       goto end_switch;                 /* with next minipacket */
     case STEP_Segmented:
       if (!MP2_mp_validate (mp, 1))
         return;                        /* continuation, from   */
       cindex = MP2_mp_move (mp, cindex, 2);    /* previous minipacket, */
       if (cindex < 0) {                /* keep recovering data   */
         mp->sequence = new_sequence & 0x3FFF;
         return;
       }
       mp->flag_1 = STEP_Continue;      /* indicate 'carry on'  */
...
{
  mp->mp_buffer = malloc (sizeof (struct MP_buffer));
  return mp->mp_buffer;
}
