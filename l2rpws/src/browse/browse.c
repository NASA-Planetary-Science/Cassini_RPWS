                       ...
                       struct DUST_STRUCT *dust_struct)
{
 ...
 /****************************************************************************/
  switch (flag) {
   default:
     ...
     break;
   case 8:
     fprintf (time_file, "    echo \"(1) thumbdust.sh\"");
     break;
  }
  fprintf (time_file, "\n");
  switch (flag) {
   default:
     ...
     break;
   case 8:
     fprintf (time_file, "    ${VOLUME_PATH}/${VOLUME_SCRIPT}/thumbdust.sh ");
     break;
  }
  ...
  if (!force_flag) {
    switch (flag) {
     default:
       ...
       break;
     case 8:
       fprintf (time_file, "    echo \"(2) thumbdust.sh\"");
       break;
    }
    fprintf (time_file, "\n");
 ...
 /****************************************************************************/
  fprintf (time_file, "    echo \"Reprocessing %s.tn.gif\"",
           build_gif_name (flag, gif_name, hfr_struct, dust_struct));
  fprintf (time_file, "\n");
  ...
  while ((c = getopt (argc, argv, "p:n:d:o:xv:flr:h")) != EOF) {
    switch (c) {
     ...
     case 'h':
       ...
       fprintf (stdout, "                Controls the appearance of the\n");
       fprintf (stdout, "                web page.  Links to next page, \n");
       fprintf (stdout, "                comment text, etc.\n");
       ...
       fprintf (stdout, "        \n");
       fprintf (stdout,
                "          DUST records are assumed to be isolated.\n");
       fprintf (stdout,
                "        Those records that occur within the current\n");
       fprintf (stdout, "        maximum delta value of %d seconds\n",
                max_delta);
       fprintf (stdout,
                "        from other WBR data will be mis-identified (usually\n");
       fprintf (stdout,
                "        resulting in a missing time series plot).\n");
       fprintf (stdout, "        \n");
       fprintf (stdout, "        \n");
       fflush (stdout);
       exit (0);
