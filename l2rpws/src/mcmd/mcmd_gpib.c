...
TYPE_TEXT, "TEXT", "Text line with <newline>",
    ...
    "hx, hx, hx, ...",
    TYPE_CNTL_RESET, "RESET", "GPIB bus reset",
    "",
    ...
    "filename",
    TYPE_READ_TEXT, "READ", "Read text from device",
    "",
    TYPE_REPLY_TEXT, "REPLY", "Read text from device",
    "quoted text",
    TYPE_REPLY_FILE, "REPLYFILE", "Read text from device",
    "file, quoted text",
    TYPE_READ_FILE, "READFILE", "Read data from device to file",
    "filename", 0, "", "", "", -1, "", "", ""};
...
static char *mcmd_gpib_help_detail (int i)
{
  static char buf[STR_LEN];
  static char *blanks = "                    ";

  if (cmd_type_table[i].type) {
    strcpy (buf, blanks);
    buf[10] = 0;
    ...
    buf[20] = 0;
    strcat (buf, "DEV, CMD, ");
    strcat (buf, cmd_type_table[i].toklist);
    strcat (buf, blanks);
    buf[48] = 0;
...
char *mcmd_gpib_help (int j)
{
  ...
  if (j < 0) {
    ...
    buf = mcmd_gpib_help_detail (j);
  }
  return buf;
}
static char *mcmd_long (int *ival, int cnt)
{
  static char string[STR_LEN];
  int i;
...
static void mcmd_ibsta (int ib_sta)
{
  static char *mne[] = { "ERR  ",
    "TIMO ",
    "END  ",
...
static void mcmd_iberr (int ib_err)
{
...
static int mcmd_iblock (int state)
{
  int old_state;
...
static int mcmd_ibfind (char *name)
{
  int device_open = 0;
...
static void mcmd_fdump (char *curve, int length)
{
  int i;
...
static int mcmd_gfile (int devid, char *name)
{
...
static int mcmd_xfile (int devid, char *filename)
{
  FILE *curve_file = NULL;
  char curve[16384];
  char line[1024];
...
static int mcmd_cfile (int devid, char *filename, int wordsize)
{
  FILE *curve_file = NULL;
  char curve[16500];
  char line[1024];
...
static int mcmd_dfile (char *filename, char *buf, int nwrd)
{
  FILE *data_file = NULL;
  int i;
  ...
  if (!data_file)
    return 0;
  i = fwrite (buf, nwrd, 1, data_file);
  fclose (data_file);
  return i;
}
static void mcmd_puts (char *buf)
{
  int i;
...
{
  static char trans[STR_LEN];
  static char trans1[STR_LEN];
  static char delim[] = { " ," };
  char *tok[TOKEN_MAX];
  char *bs, *es;
  char *string;
  ...
  mcmd_delay (delay_period);
  switch (cmd_type) {
   case TYPE_REPLY_TEXT:
     memset (trans1, 0, STR_LEN);
     k = 0;
   ...
   case TYPE_READ_TEXT:
     memset (trans1, 0, STR_LEN);
     devsts = ibrd (devid, trans1, STR_LEN - 1);
     if (devsts & ERR) {
       mcmd_iberr (iberr);
       mcmd_ibsta (devsts);
       trans1[strlen (trans1) - 1] = 0;
   ...
   case TYPE_READ_FILE:
     {
       ...
       len = strtol (tok[2], NULL, 0);
       buf = malloc (len + 1);
       if (!buf)
         break;
       nwr = ibrd (devid, buf, len);
       i = mcmd_dfile (tok[3], buf, nwr);
       if (devsts & ERR) {
         mcmd_iberr (iberr);
         mcmd_ibsta (devsts);
         trans1[strlen (trans1) - 1] = 0;
