/* Really, you name the header file containing the basic definitions for
 * RPWS packets "rtiu.h" (Real Time Interface Unit) instead of something like
 * packet_defs.h because you are too lazy to edit the files that include the
 * header after you moved on from the bench test phase of the mission.
 *
 * Wow, you guys really don't give a flying **** about doing it right, do
 * you.  
 *
 *  -- piker 2015-12-23
 */
...
#define DATA_MPP_DUST    0x10B          /* DUST packet                 */
#define DATA_MPP_MRO     0x10D          /* MRO packet                  */
...
#define DATA_MPS_MRO     0x20D          /* MRO packet                  */
...
#define DATA_MPL_MRO     0x30D          /* MRO packet                  */
...
#define DATA_MPC_MRO     0x40D          /* MRO packet                  */
...
#define STATUS_MPUS_ERROR_MASK	0x80000000
#define STATUS_MPUS_ERROR	0x80000000
#define STATUS_MPUS_VALID	0x00000000
...
#define STREAM_TYPE_lrs   1
#define STREAM_TYPE_hrs   2
#define STREAM_TYPE_hsk   4
#define STREAM_TYPE_mro   8

/********************************************************
 *   user-variable.packet.ancillary.ancil		*
 *   user-variable.packet.ancillary.biust		*
 *   user-variable.packet.mpus.mpus_status		*
 ********************************************************/
struct MPUS_header
{
  unsigned char filler1[204];
  unsigned long mpus_status;
};
 ...
 *   user-variable.packet.cds_header.field		*
 ********************************************************/
struct CDS_packet
{
  unsigned char filler1[256];
  unsigned char header[12];
  unsigned char mp_data[CDS_PKTMAX - 12 + 256];
};
struct CDS_header
{
  unsigned char filler1[256];
 ...
 *	user-variable.packet.chdo_ancillary.type_92.xxx	*
 *	user-variable.packet.chdo_ancillary.type_94...	*
 ********************************************************/
struct event_time
{
...
struct CHDO_type_92
{
  unsigned short chdo_type;             /*  0  */
  unsigned short chdo_length;           /*  2  */
  unsigned char originator;             /*  4  */
  unsigned char last_modifier;          /*  5  */
  unsigned char scft_id;                /*  6  */
  unsigned char data_source;            /*  7  */
  unsigned short status_flags;          /*  8  */
  unsigned short ert[3];                /* 10  */
  unsigned char rs_codeword_status;     /* 16  */
  unsigned char frame_extract_count;    /* 17  */
  unsigned short dsn_record_seq[2];     /* 18  */
  unsigned char bet;                    /* 22  */
  unsigned char fly;                    /* 23  */
  unsigned char decode_status;          /* 24  */
  unsigned char decode_method;          /* 25  */
  unsigned char sync_flags;             /* 26  */
  unsigned char pn_errors;              /* 27  */
  unsigned char virtual_channel_id;     /* 28  */
  unsigned char virtual_frame_count;    /* 29  */
  unsigned char frame_hdr_error_flag;   /* 30  */
  unsigned char rs_decode_errors;       /* 31  */
  unsigned char spare_0;                /* 32  */
  unsigned char frequency_band;         /* 33  */
  unsigned char bit_rate[4];            /* 34  */
  unsigned char spare_1[2];             /* 38  */
  unsigned char snt[4];                 /* 40  */
  unsigned char ssnr[4];                /* 44  */
  unsigned char signal_level[4];        /* 48  */
  unsigned char antennas;               /* 52  */
  unsigned char receivers;              /* 53  */
  unsigned char master_antenna;         /* 54  */
  unsigned char master_receiver;        /* 55  */
  unsigned char dtm_group;              /* 56  */
  unsigned char tlm_channel;            /* 57  */
  unsigned short lock_status;           /* 58  */
  unsigned char version;                /* 60  */
  unsigned char build;                  /* 61  */
  unsigned char orig_source;            /* 62  */
  unsigned char curr_source;            /* 63  */
  unsigned short rct[3];                /* 64  */
  unsigned short anomaly_flags;         /* 70  */
  unsigned short lock_count;            /* 72  */
  unsigned short lrn;                   /* 74  */
  unsigned short pub[3];                /* 76  */
  unsigned short block_v_rcvrs;         /* 82  */
};                                      /* 84  */
struct CHDO_type_94
{
  unsigned short chdo_type;             /*  0 */
  unsigned short chdo_length;           /*  2 */
  unsigned short cds_error_flags;       /*  4 */
  unsigned short source_pkt_seq_count;  /*  6 */
  unsigned short non_fill_length;       /*  8 */
  unsigned short packet_apid;           /* 10  */
  unsigned short superpkt_seq_cnt;      /* 12  */
  unsigned short cas_sclk[3];           /* 14  */
  unsigned short scet[3];               /* 20  */
  unsigned char probe_rs_errors;        /* 32  */
  unsigned char scet_flags;             /* 33  */
  unsigned short spare_0;               /* 34  */
};                                      /* 36  */

struct CHDO_Ancillary
{
...
struct WS_Time_Tag
{
  ...
  *******************************************************/
#define COMPRESS_METHOD_unknown		0
#define COMPRESS_METHOD_MFR		1
#define COMPRESS_METHOD_DCP		2
#define COMPRESS_METHOD_DCP_WALSH	3
#define COMPRESS_METHOD_DCP_RICE	4
#define COMPRESS_METHOD_DCP_WALSH_RICE	5
#define COMPRESS_METHOD_DCC		6
#define COMPRESS_METHOD_MEANDER		7
struct COMPRESS_status
{
 ...
 /*******************************************************
  *	Data Stream Indicator indicator			*
  *							*
  *	user-variable.packet.stream.CDS_stream		*
  *	user-variable.packet.stream.CDS_fill_flag	*
  *	user-variable.packet.stream.CCSDS		*
  *******************************************************/
#define	CDS_FILL_FLAG_1		0x01
#define	CDS_FILL_FLAG_2		0x02
#define	CDS_FILL_FLAG		0x08
#define	CDS_SEQUENCE_FLAG	0x04
#define	CDS_FILL_FLAG_MP	0x10
struct STREAM_status
{
  unsigned char filler1[44];
  char CDS_stream;
  char CDS_fill_flag;
  char CCSDS[2];
};
...
struct CDS_buffer
{
  ...
  union
  {
    ...
    struct CDS_header cds_header;
    struct MPUS_header mpus;
    ...
    struct CHDO_Ancillary chdo_ancillary;
    struct LENGTH_status index;
    struct STREAM_status stream;
    struct MPP mpp;
    struct MPS mps;
    struct PKT pkt;
  } packet;
...
struct GENERIC_buffer
{
  ...
  long record_type;                     /*   4  */
  long status;                          /*   8  */
  char cds_time_tag_a[8];               /*  12  time of 1st. segment */
  char cds_time_tag_b[8];               /*  20  time of last segment */
  char ws_time_tag_a[8];                /*  28  when data arrived in WS */
  char ws_time_tag_b[8];                /*  36  when data last formatted in WS */
  char compress_method[4];              /*  44  */
  char compress_bit_count[4];           /*  48  */
  char compress_result[4];              /*  52  */
  char stream_info[4];                  /*  56  Source stream information */
  char length_data_start[4];            /*  60  unrestricted compressed m.p. length */
  char length_data_length[4];           /*  64  unrestricted minipacket length */
  char segment_count[4];                /*  68  */
  char segment_number[4];               /*  72  */
  char record_count[4];                 /*  76  */
  char record_number[4];                /*  80  */
  char epoch[4];                        /*  84  epoch of data */
  char sequence[4];                     /*  88  sequence field from cds record */
  char chdo_type_92[84];                /*  92  80 bytes, up to here */
  char chdo_type_94[36];                /* 176  164 to here */
  char filler[4];                       /* 212  4 spares 4/5/04 */
  char mpus_status[4];                  /* 216  */
  char ancil[8];                        /* 220  ancillary broadcast, when available */
  char biust[8];                        /* 228  biu status words, when available */
  char f_sclk[8];                       /* 236  */
  char f_scet[8];                       /* 244  */
  char f_ert[8];                        /* 252  */
  char f_rct[8];                        /* 260  268 256+12 */
};                                      /* 268  (start of data) */
#endif
