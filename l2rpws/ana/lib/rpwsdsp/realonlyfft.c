...
int realonlyfft(float *xtime,float *magphs,int n,float avg )
{
...
*/
  ...
  for(i=0;i<(n/2);i++){
    magphs[i] = sqrt( xreal[i]*xreal[i] + ximag[i]*ximag[i] );
    if(xreal[i] || ximag[i]){		/* Phase Information */ 
      magphs[i+(n/2)] = atan2(ximag[i],xreal[i]);
      }
    else{
      magphs[i+(n/2)] = 0.0;
      }
