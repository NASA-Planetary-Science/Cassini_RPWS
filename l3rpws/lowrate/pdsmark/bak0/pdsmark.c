...
*/
  ...
  while(--argc){
    ...
    if(!strcmp("-d",*argv)){
      ...
      nLfdrMode=0xFFFFFFFF;
    }/* else lfdr */
    else if(!strcmp("-mfdr",*argv)){
      nMfdrMode=0xFFFFFFFF;
    }/* else mfdr */
...
*/
  ...
  while((p=fgets(s,256,h))!=NULL){
    pList->sName[pList->nMax];
    nSclk=pList->nSclk[pList->nMax];
    nRti=pList->nRti[pList->nMax];
    pList->nEvt[pList->nMax]=GetEventTime(nSclk,nRti);
  }
